...
Load this reference only when inspecting or seeding local Piku state directly.
...
```bash
node apps/server/scripts/piku-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/piku-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/piku-seed.sql
```
