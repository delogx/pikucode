...
Define the long-term server auth architecture for Piku Code before first-class remote environments ship.
...
Every privileged surface of the Piku server must go through the same auth policy engine:
...
Participants:
  ...
  PikuServer      = local backend child process
...
DesktopMain -> PikuServer : spawn server (--bootstrap-fd ...)
DesktopMain -> PikuServer : send desktop bootstrap envelope
note over PikuServer : policy = DesktopManagedLocalPolicy
note over PikuServer : allowed pairing = desktop-bootstrap only

Frontend -> DesktopMain : request local bootstrap grant
DesktopMain --> Frontend : short-lived desktop bootstrap grant

Frontend -> PikuServer : POST /api/auth/bootstrap
PikuServer -> PikuServer : validate desktop bootstrap grant
PikuServer -> PikuServer : create browser session
PikuServer --> Frontend : Set-Cookie: session=...

Frontend -> PikuServer : GET /ws + authenticated cookie
PikuServer -> PikuServer : validate cookie session
PikuServer --> Frontend : websocket accepted
```
...
```text
Participants:
  UserShell     = npx piku launcher
  PikuServer      = standalone local server
  Browser       = browser tab

UserShell -> PikuServer : start server
PikuServer -> PikuServer : getOrCreate("server-signing-key")
note over PikuServer : policy = LoopbackBrowserPolicy

UserShell -> PikuServer : issue one-time pairing token
PikuServer --> UserShell : pairing URL or pairing token

UserShell --> Browser : open /pair?token=...

Browser -> PikuServer : GET /pair?token=...
PikuServer -> PikuServer : validate one-time token
PikuServer -> PikuServer : create browser session
PikuServer --> Browser : Set-Cookie: session=...
PikuServer --> Browser : redirect to app

Browser -> PikuServer : GET /ws + authenticated cookie
PikuServer --> Browser : websocket accepted
```
...
Participants:
  ...
  PikuServer      = Piku server
  PhoneBrowser  = mobile browser

DesktopUser -> DesktopMain : enable remote access via tunnel
DesktopMain -> PikuServer : switch policy to RemoteReachablePolicy
DesktopMain -> Tunnel : publish local Piku endpoint
Tunnel --> DesktopMain : public https/wss URL

DesktopMain -> PikuServer : issue one-time pairing token
PikuServer --> DesktopMain : pairing token
...
Tunnel -> PikuServer : forward request
PikuServer -> PikuServer : validate one-time token
PikuServer -> PikuServer : create mobile browser session
PikuServer --> PhoneBrowser : Set-Cookie: session=...
PikuServer --> PhoneBrowser : redirect to app

PhoneBrowser -> Tunnel : GET /ws + authenticated cookie
Tunnel -> PikuServer : forward websocket upgrade
PikuServer --> PhoneBrowser : websocket accepted
```
...
DesktopUser -> PikuServer : enable private-network access
PikuServer -> PikuServer : switch policy to RemoteReachablePolicy
DesktopUser -> PikuServer : issue one-time pairing token
PikuServer --> DesktopUser : pairing URL / QR

DesktopUser -> PhoneBrowser : open private-network URL
PhoneBrowser -> PrivateNet : GET http(s)://private-host/pair?token=...
PrivateNet -> PikuServer : route request
PikuServer -> PikuServer : validate one-time token
PikuServer -> PikuServer : create mobile browser session
PikuServer --> PhoneBrowser : Set-Cookie: session=...
PikuServer --> PhoneBrowser : redirect to app
...
PrivateNet -> PikuServer : websocket upgrade
PikuServer --> PhoneBrowser : websocket accepted
```
...
Participants:
  ...
  RemotePiku      = remote Piku server
  Frontend      = desktop renderer

DesktopUser -> DesktopMain : add SSH host
DesktopMain -> SSH : connect to remote host
SSH -> RemoteHost : probe environment / verify piku availability
DesktopMain -> SSH : run remote launch command
SSH -> RemoteHost : piku remote launch --json
RemoteHost -> RemotePiku : start or reuse server
RemotePiku --> RemoteHost : port + environment metadata
...
note over RemotePiku : policy = RemoteReachablePolicy
note over DesktopMain,RemotePiku : desktop may use a trusted bootstrap flow here

Frontend -> DesktopMain : request bootstrap for selected environment
DesktopMain --> Frontend : short-lived bootstrap grant

Frontend -> RemotePiku : POST /api/auth/bootstrap via forwarded port
RemotePiku -> RemotePiku : validate bootstrap grant
RemotePiku -> RemotePiku : create browser session
RemotePiku --> Frontend : Set-Cookie: session=...

Frontend -> RemotePiku : GET /ws + authenticated cookie
RemotePiku --> Frontend : websocket accepted
```
