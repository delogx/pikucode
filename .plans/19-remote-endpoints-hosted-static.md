...
- users can expose a backend through LAN, their own Tailscale, MagicDNS, a manual HTTPS endpoint, or later Piku Tunnel
- users can generate a hosted pairing link for `app.pikucode.dev`
...
This plan intentionally leaves the paid Piku cloud tunnel fabric out of scope. It defines the OSS foundation that Piku Tunnel should later plug into.
...
- `https://app.pikucode.dev` cannot reliably call `http://...` or `ws://...` endpoints because browsers block mixed content.
...
- Generate `app.pikucode.dev` links only from endpoints that are likely hosted-browser compatible, or show a warning with an explicit fallback.
...
```text
https://app.pikucode.dev/pair?host=<encoded endpoint httpBaseUrl>#token=<one-time token>
```
...
- HTTP private-network endpoints are marked incompatible with `app.pikucode.dev`.
...
- `app.pikucode.dev` works as a real client shell.
...
- `app.pikucode.dev` can pair a reachable HTTPS backend and reconnect after reload.
- A saved environment can be used without any backend at `app.pikucode.dev`.
...
## Phase 4: Future Piku Tunnel Provider
...
- create endpoint with `provider: "piku-tunnel"`
- surface tunnel status
- provide stable HTTPS URL
- use existing backend pairing/session auth
- never bypass server auth

The tunnel fabric can later be Pipenet-derived, Tailscale-derived, or another reverse tunnel implementation. The rest of Piku Code should only see an `AdvertisedEndpoint`.
