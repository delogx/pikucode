...
- review threads/comments/diffs: key by provider CR ID and head SHA/version when available; fetch on demand for Piku Review
...
- The provider API includes the review operations needed by future Piku Review work, even if they are capability-gated.
