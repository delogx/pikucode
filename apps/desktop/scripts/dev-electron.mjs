...
const remoteDebuggingPort = process.env.PIKU_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line pikucode/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.PIKU_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.PIKU_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--piku-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--piku-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
