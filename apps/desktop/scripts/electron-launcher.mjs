// This file mostly exists because we want dev mode to say "Piku Code (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Piku Code (Dev)" : "Piku Code (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `dev.pikucode.app.dev.${devBundleIdSuffix || "local"}`
  : "dev.pikucode.app";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["piku-dev"] : ["pikucode"];
...
// oxlint-disable-next-line pikucode/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentLauncherScript({
  ...
  const envEntries = [
    ...
    ["PIKU_PORT", environment.PIKU_PORT],
    ["PIKU_HOME", environment.PIKU_HOME],
    ["PIKU_COMMIT_HASH", environment.PIKU_COMMIT_HASH],
    ["PIKU_OTLP_TRACES_URL", environment.PIKU_OTLP_TRACES_URL],
    ["PIKU_OTLP_EXPORT_INTERVAL_MS", environment.PIKU_OTLP_EXPORT_INTERVAL_MS],
    ["PIKU_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --piku-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Piku Code name.
