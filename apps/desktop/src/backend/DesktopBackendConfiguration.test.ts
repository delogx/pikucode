    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          ...
          PIKU_PORT: "9999",
          PIKU_MODE: "desktop",
          PIKU_DESKTOP_LAN_HOST: "192.168.1.50",
...
const withHarness = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "piku-desktop-backend-config-test-",
    });
...
describe("DesktopBackendConfiguration", () => {
  it.effect("resolvePrimary produces a stable scoped bootstrap token", () =>
    withHarness(
      Effect.gen(function* () {
        ...
        assert.isUndefined(first.env.PIKU_PORT);
        assert.isUndefined(first.env.PIKU_MODE);
        assert.isUndefined(first.env.PIKU_DESKTOP_LAN_HOST);
        ...
        assert.equal(first.bootstrap.pikuHome, environment.baseDir);
  ...
  it.effect("prefers the external packaged resource monitor over the copy inside the asar", () =>
    Effect.gen(function* () {
      ...
      const embeddedMonitorPath = `${resourcesPath}/app.asar/apps/desktop/prod-resources/resource-monitor/piku-resource-monitor`;
      const monitorPath = `${resourcesPath}/resource-monitor/piku-resource-monitor`;
  ...
  it.effect("prefers the release resource monitor when both development builds exist", () =>
    Effect.gen(function* () {
      ...
      const releaseMonitorPath = path.join(
        baseDir,
        "native/resource-monitor/target/release/piku-resource-monitor",
      );
      const debugMonitorPath = path.join(
        baseDir,
        "native/resource-monitor/target/debug/piku-resource-monitor",
      );
  ...
  it.effect("resolvePrimaryLabel reports the local environment", () =>
    withHarness(
      Effect.gen(function* () {
        const configuration = yield* DesktopBackendConfiguration.DesktopBackendConfiguration;
