...
export const getLocalEnvironmentBootstraps = DesktopIpc.makeSyncIpcMethod({
  ...
  handler: Effect.fn("desktop.ipc.window.getLocalEnvironmentBootstraps")(function* () {
    ...
    for (const instance of instances) {
      ...
      // (mid-registration, before its first start cycle) is not listening on a
      // port. We surface it as a *pending* bootstrap (null endpoints, no token)
      // so the renderer can show a "Connecting…" indicator while it retries —
      // null endpoints keep the renderer from dialing the dead port, avoiding
      // the needless /api/auth/bootstrap/bearer error cycles a real endpoint
      // would trigger.
      if (Option.isNone(config)) {
        // Skip the primary (same-origin, no "connecting" affordance).
