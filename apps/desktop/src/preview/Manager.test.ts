...
import type { DesktopPreviewRecordingFrame } from "@piku/contracts";
...
const browserSessionLayer = Layer.succeed(
  ...
  BrowserSession.BrowserSession.of({
    getPartition: () => Effect.succeed("persist:pikucode-preview-test"),
    isPartition: (partition) => partition.startsWith("persist:pikucode-preview-"),
...
const environmentLayer = Layer.succeed(
  ...
  DesktopEnvironment.DesktopEnvironment.of({
    browserArtifactsDir: "/tmp/piku/dev/browser-artifacts",
    dirname: "/tmp/piku/desktop",
    path: {
...
describe("PreviewManager", () => {
  ...
  effectIt.effect("captures a PNG screenshot into browser artifacts", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        ...
        expect(capturePage).toHaveBeenCalledOnce();
        expect(mkdir).toHaveBeenCalledWith("/tmp/piku/dev/browser-artifacts");
        ...
        expect(browserWindowConstructor).toHaveBeenCalledWith(
          expect.objectContaining({
            ...
            webPreferences: expect.objectContaining({
              preload: "/tmp/piku/desktop/preview-pip-preload.cjs",
    ...
    withManager((manager) =>
      Effect.gen(function* () {
        yield* manager.revealArtifact(
          "/tmp/piku/dev/browser-artifacts/browser-screenshot-test.png",
        );

        expect(showItemInFolder).toHaveBeenCalledWith(
          "/tmp/piku/dev/browser-artifacts/browser-screenshot-test.png",
        );
        const exit = yield* Effect.exit(manager.revealArtifact("/tmp/piku/dev/settings.json"));
        expect(Exit.isFailure(exit)).toBe(true);
        if (Exit.isSuccess(exit)) return;
        const error = Option.getOrThrow(Cause.findErrorOption(exit.cause));
        expect(error).toMatchObject({
          _tag: "PreviewArtifactPathOutsideDirectoryError",
          artifactPath: "/tmp/piku/dev/settings.json",
          artifactDirectory: "/tmp/piku/dev/browser-artifacts",
        });
  ...
  effectIt.effect("copies screenshot artifacts to the system clipboard", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        const artifactPath = "/tmp/piku/dev/browser-artifacts/browser-screenshot-test.png";
        ...
        const exit = yield* Effect.exit(
          manager.copyArtifactToClipboard("/tmp/piku/dev/settings.json"),
        );
        expect(Exit.isFailure(exit)).toBe(true);
        if (Exit.isSuccess(exit)) return;
        const error = Option.getOrThrow(Cause.findErrorOption(exit.cause));
