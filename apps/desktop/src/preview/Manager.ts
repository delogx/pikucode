...
import { normalizePreviewUrl } from "@piku/shared/preview";
...
const makeNativeOperations = Effect.fn("PreviewManager.makeOperations")(function* (
  ...
  const ensurePlaywrightInjected = Effect.fn("PreviewManager.ensurePlaywrightInjected")(function* (
    ...
    const installed = yield* evaluateWithDebugger<boolean>(
      tabId,
      send,
      "Boolean(globalThis.__pikuPlaywrightInjected)",
      true,
    );
    if (installed) return;
  ...
  const resolveClickPoint = Effect.fn("PreviewManager.resolveClickPoint")(function* (
    ...
    >(
      ...
      `(() => {
          try {
            const injected = globalThis.__pikuPlaywrightInjected;
  ...
  const typeIntoAutomationTarget = Effect.fn("PreviewManager.typeIntoAutomationTarget")(function* (
    ...
    >(
      ...
      `(() => {
          try {
            const element = ${locatorJson ? `(() => { const injected = globalThis.__pikuPlaywrightInjected; return injected.querySelector(injected.parseSelector(${locatorJson}), document, true); })()` : "document.activeElement"};
            if (!element) return { notFound: true };
            const textControl =
  ...
  const performAutomationScroll = Effect.fn("PreviewManager.performAutomationScroll")(function* (
    ...
    >(
      ...
      `(() => {
        try {
          const target = ${locatorJson ? `(() => { const injected = globalThis.__pikuPlaywrightInjected; return injected.querySelector(injected.parseSelector(${locatorJson}), document, true); })()` : "window"};
  ...
  const performAutomationWaitFor = Effect.fn("PreviewManager.performAutomationWaitFor")(function* (
    ...
    while ((yield* currentMillis) <= deadline) {
      ...
      >(
        ...
        `(() => {
              try {
                const selectorMatched = ${locatorJson ? `(() => { const injected = globalThis.__pikuPlaywrightInjected; return injected.querySelector(injected.parseSelector(${locatorJson}), document, false) !== null; })()` : "true"};
                const textMatched = ${
...
>()("@piku/desktop/preview/Manager/PreviewManager") {}
