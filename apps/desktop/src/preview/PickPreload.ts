...
const OVERLAY_ATTRIBUTE = "data-pikucode-annotation-ui";
const Z_INDEX_OVERLAY = 2147483646;
const PRIMARY = "var(--piku-primary)";
const PRIMARY_FILL = "color-mix(in srgb, var(--piku-primary) 10%, transparent)";
...
const applyAnnotationTheme = (
  ...
  const variables = {
    "--piku-radius": theme.radius,
    "--piku-background": theme.background,
    "--piku-foreground": theme.foreground,
    "--piku-popover": theme.popover,
    "--piku-popover-foreground": theme.popoverForeground,
    "--piku-primary": theme.primary,
    "--piku-primary-foreground": theme.primaryForeground,
    "--piku-muted": theme.muted,
    "--piku-muted-foreground": theme.mutedForeground,
    "--piku-accent": theme.accent,
    "--piku-accent-foreground": theme.accentForeground,
    "--piku-border": theme.border,
    "--piku-input": theme.input,
    "--piku-ring": theme.ring,
    "--piku-font-sans": theme.fontSans,
    "--piku-font-mono": theme.fontMono,
  };
...
function startAnnotation(): void {
  ...
  const refreshToolButtons = (): void => {
    ...
    document.documentElement.setAttribute("data-pikucode-annotation-tool", tool);
  };
      ...
      if (isUsableRect(rect)) {
        ...
        if (found === 0) {
          ...
          const regionBox = createBox(
            PRIMARY,
            "color-mix(in srgb, var(--piku-primary) 6%, transparent)",
          );
  ...
  const teardown = (notifyMain: boolean): void => {
    ...
    document.documentElement.removeAttribute("data-pikucode-annotation-tool");
    cursorStyle.remove();
    host.remove();
    activeSession = null;
