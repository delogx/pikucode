...
const shouldLaunchElectronAfterPack = process.env.PIKU_DESKTOP_DEV === "1";
  ...
  run: {
    tasks: {
      build: {
        ...
        dependsOn: ["piku#build"],
        cache: false,
      },
      dev: {
        command:
          "node scripts/build-preview-annotation-css.mjs && cross-env PIKU_DESKTOP_DEV=1 vp pack --watch",
        dependsOn: ["piku#build"],
        cache: false,
      },
      "dev:bundle": {
  ...
  pack: [
    {
      ...
      clean: true,
      deps: {
        alwaysBundle: (id) => id.startsWith("@piku/"),
      },
