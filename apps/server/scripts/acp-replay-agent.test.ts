...
async function runRuntimeExit(status: "success" | "error" | "cancelled") {
  const scratch = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "piku-acp-replay-agent-"));
  ...
  const child = NodeChildProcess.spawn(process.execPath, args, {
    env: {
      ...process.env,
      PIKU_ACP_REPLAY_STATUS_PATH: statusPath,
      PIKU_ACP_REPLAY_TRANSCRIPT: Buffer.from(JSON.stringify(transcript), "utf8").toString(
        "base64",
      ),
      PIKU_ACP_REPLAY_WORKSPACE: scratch,
    },
