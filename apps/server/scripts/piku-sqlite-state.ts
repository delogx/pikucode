...
// @effect-diagnostics nodeBuiltinImport:off - node:os resolves the shared Piku home guard.
...
export class SqliteStateDatabaseMissingError extends Schema.TaggedErrorClass<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start Piku once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedErrorClass<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.pikucode database. Use an isolated --base-dir.";
  }
}
...
export const runSqliteState = Effect.fn("runSqliteState")(function* (
  ...
  const sharedHome = path.resolve(options.sharedHome ?? path.join(NodeOS.homedir(), ".pikucode"));
...
export const t3SqliteStateCommand = Command.make(
  "piku-sqlite-state",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      Flag.withDescription("Explicit Piku base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated Piku SQLite database with automatic backups for writes.",
  ),
);
