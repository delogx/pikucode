...
function selectedQueryMode(defaultMode: ClaudeRecordingQueryMode): ClaudeRecordingQueryMode {
  const raw = readArgValue("--query-mode") ?? process.env.PIKU_CLAUDE_REPLAY_QUERY_MODE;
  if (raw === undefined) {
    return defaultMode;
  }
  if (
...
const scenario = readArgValue("--scenario") ?? process.env.PIKU_CLAUDE_REPLAY_SCENARIO ?? "simple";
...
function selectedPrompts(): ReadonlyArray<string> {
  if (process.env.PIKU_CLAUDE_REPLAY_PROMPTS !== undefined) {
    return process.env.PIKU_CLAUDE_REPLAY_PROMPTS.split("\n---\n").filter(
      (prompt) => prompt.length > 0,
    );
  }
  if (process.env.PIKU_CLAUDE_REPLAY_PROMPT !== undefined) {
    return [process.env.PIKU_CLAUDE_REPLAY_PROMPT];
  }
...
const cwd =
  process.env.PIKU_CLAUDE_REPLAY_CWD ??
...
try {
  ...
  const transcript = await recordClaudeAgentSdkReplayTranscript({
    ...
    modelSelection: {
      ...
      model: process.env.PIKU_CLAUDE_REPLAY_MODEL ?? CLAUDE_MODEL_SELECTION.model,
    },
    cwd,
    ...(process.env.PIKU_CLAUDE_REPLAY_SESSION_ID === undefined
      ? {}
      : { sessionId: process.env.PIKU_CLAUDE_REPLAY_SESSION_ID }),
    queryMode,
