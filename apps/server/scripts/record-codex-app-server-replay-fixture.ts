...
const CODEX_REPLAY_PLAN_MODE_DEVELOPER_INSTRUCTIONS =
  process.env.PIKU_CODEX_REPLAY_PLAN_DEVELOPER_INSTRUCTIONS ??
...
function parseScenarios(): ReadonlyArray<ScenarioName> {
  const rawValues = [
    ...
    ...(process.env.PIKU_CODEX_REPLAY_SCENARIOS ? [process.env.PIKU_CODEX_REPLAY_SCENARIOS] : []),
  ];
...
function makeCodexLayer({ recorder }: { readonly recorder: Recorder }) {
  ...
  return Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const commandName = environment.PIKU_CODEX_BIN ?? environment.CODEX_BIN ?? "codex";
...
const program = Effect.gen(function* () {
  ...
  const outDir = readArgValue("--out-dir") ?? process.env.PIKU_CODEX_REPLAY_OUT_DIR;
  const singleOutPath = readArgValue("--out") ?? process.env.PIKU_CODEX_REPLAY_OUT;
  ...
  if (singleOutPath && selected.length !== 1) {
    throw new Error("--out / PIKU_CODEX_REPLAY_OUT can only be used with exactly one --scenario.");
  }
