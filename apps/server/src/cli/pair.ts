/**
 * `piku pair` - mint a pairing token for an already-running server and print it
 ...
 * own `.pikucode` is checked first (matching dev-runner precedence); otherwise the
 * shared Piku home.
 */
import { AuthStandardClientScopes, ExecutionEnvironmentDescriptor } from "@piku/contracts";
import { resolveWorktreePikuHome } from "@piku/shared/devHome";
...
const WELL_KNOWN_ENVIRONMENT_PATH = "/.well-known/piku/environment";
...
export class NoRunningServerError extends Schema.TaggedErrorClass<NoRunningServerError>()(
  ...
  override get message(): string {
    return [
      "No running Piku Code server found.",
      ...this.checkedStatePaths.map((statePath) => `  checked ${statePath}`),
      "Start one with `npx piku serve`, or connect this machine with Piku Connect: `npx piku connect`.",
    ].join("\n");
  }
}
...
/**
 * Three outcomes, because they drive different decisions: a Piku descriptor
 * (pair with it), nothing answering, or something answering that is not a Piku
 * server.
 */
type EnvironmentProbeResult =
  | { readonly _tag: "descriptor"; readonly descriptor: ExecutionEnvironmentDescriptor }
  | { readonly _tag: "unreachable" }
  | { readonly _tag: "not-a-piku-server" };
...
const probeEnvironmentDescriptor = (
  ...
  Effect.gen(function* () {
    ...
    // a stale mapping, not a live occupant.
    ...
    const descriptor = yield* HttpClientResponse.filterStatusOk(response).pipe(
      ...
      Effect.mapError(() => ({ _tag: "not-a-piku-server" }) as const),
    );
...
const discoverPairTarget = Effect.fn("pair.discoverPairTarget")(function* (
  ...
  if (explicitBaseDir !== undefined && explicitBaseDir.trim().length > 0) {
    ...
    // Same precedence as dev-runner: inside a linked worktree its own `.pikucode`
    // outranks the shared home, so `piku pair` in a worktree pairs with the dev
    // server under test rather than the daily-driver install.
    const worktreeHome = yield* resolveWorktreePikuHome(process.cwd());
    if (worktreeHome !== undefined) {
      bases.push(worktreeHome);
    }
    const envHome = yield* Config.string("PIKU_HOME").pipe(Config.option);
...
const mintPairingLink = Effect.fn("pair.mintPairingLink")(function* (input: {
  ...
  return yield* Effect.gen(function* () {
    ...
    return yield* environmentAuth.createPairingLink({
      ...
      label: Option.getOrElse(input.label, () => "piku pair"),
...
export const pairCommand = Command.make("pair", {
  ...
  Command.withDescription(
    "Mint a pairing token for a running Piku Code server and print it as a QR code.",
  ),
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      if (isLoopbackHost(new URL(pairingBaseUrl).hostname)) {
        notes.push(
          "This URL is only reachable from this machine. Restart the server with a reachable --host, or connect this machine with Piku Connect: `npx piku connect`.",
        );
      }
