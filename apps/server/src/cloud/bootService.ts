...
/**
 * Installs Piku Code as a per-user boot service. Linux-only for now: systemd
 * user unit + loginctl enable-linger. The service runs a stable or pinned
 * runtime — never an ephemeral `npx piku` cache whose eviction could break
 * startup.
 */

const BOOT_SERVICE_NAME = "pikucode";

export const BOOT_SERVICE_UNIT_FILE = `${BOOT_SERVICE_NAME}.service`;
export const BOOT_SERVICE_UNIT_ENV = "PIKU_BOOT_SERVICE_UNIT";
...
/**
 * `npx piku` (and pnpm dlx / bunx) run out of ephemeral package-manager
...
export interface BootServicePlan {
  ...
  /** Absolute path of the pinned piku entry point the unit will run. */
  readonly pikuEntryPath: string;
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    ...
    "Type=simple",
    "WorkingDirectory=%h",
    `Environment=PIKU_HOME=${quoteSystemdValue(plan.baseDir)}`,
    `Environment=${BOOT_SERVICE_UNIT_ENV}=${BOOT_SERVICE_UNIT_FILE}`,
    `ExecStart=${quoteSystemdValue(plan.nodePath)} ${quoteSystemdValue(plan.pikuEntryPath)} serve`,
    "Restart=always",
    "RestartSec=5",
...
export class BootServiceInstallError extends Schema.TaggedErrorClass<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the Piku Code background service.";
  }
}
...
export const make = Effect.fn("cloud.boot_service.make")(function* (input: {
   ...
   * of bin.mjs) because piku ships native deps like node-pty.
   */
  ...
  const plan: BootServicePlan = {
    ...
    pikuEntryPath: plannedEntryPath,
    baseDir: input.baseDir,
    logPath,
    unitPath,
  };
  ...
  const status: BootService["Service"]["status"] = Effect.gen(function* () {
    ...
    // references still exists (a pinned runtime under ~/.pikucode can be deleted to
