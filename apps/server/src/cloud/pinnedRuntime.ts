...
/**
 * A pinned runtime is an exact `piku@<version>` npm-installed into
 ...
 * switching over — never `npx piku`, whose cache is ephemeral and whose
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, "node_modules", "piku", "dist", "bin.mjs"),
...
/**
 * Installs `piku@<version>` into the pinned runtime directory unless a complete
  ...
  function* (input: {
    ...
    return yield* pinnedRuntimeInstallLock.withPermit(
      Effect.gen(function* () {
        ...
        const installStep = "installing the pinned piku runtime (this can take a few minutes)";
        yield* runner
          .run({
            command: "npm",
            args: [
