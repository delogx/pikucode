...
it.effect("prefers a runtime relay URL override over the statically injected value", () =>
  Effect.gen(function* () {
    const relayUrl = yield* makeRelayUrlConfig("https://embedded.example.test").pipe(
      provideEnv({ PIKU_RELAY_URL: "https://runtime.example.test///" }),
    );
...
it.effect("rejects an insecure runtime relay URL override", () =>
  makeRelayUrlConfig("https://embedded.example.test").pipe(
    provideEnv({ PIKU_RELAY_URL: "http://runtime.example.test" }),
    Effect.flip,
  ),
);
  ...
  Effect.gen(function* () {
    assert.equal(
      yield* hostedAppUrlConfig.pipe(
        provideEnv({ PIKU_HOSTED_APP_URL: "https://nightly.app.pikucode.dev" }),
      ),
      "https://nightly.app.pikucode.dev",
    );
    assert.equal(
      yield* hostedAppUrlConfig.pipe(provideEnv({ PIKU_HOSTED_APP_URL: "http://localhost:5733" })),
      "http://localhost:5733",
    );
  }),
);
...
it.effect("rejects malformed or insecure hosted app URLs", () =>
  Effect.gen(function* () {
    for (const value of [
      "app.pikucode.dev",
      "http://app.pikucode.dev",
      "https://app.pikucode.dev/nested",
      "https://app.pikucode.dev?alias=true",
    ]) {
      const result = yield* hostedAppUrlConfig.pipe(
        provideEnv({ PIKU_HOSTED_APP_URL: value }),
        Effect.result,
      );
...
it.effect("prefers runtime Clerk OAuth config overrides over statically injected values", () =>
  Effect.gen(function* () {
    const config = yield* makeCloudCliOAuthConfig({
      ...
      provideEnv({
        PIKU_CLERK_PUBLISHABLE_KEY: "pk_test_cnVudGltZS5leGFtcGxlLnRlc3Qk",
        PIKU_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_client_runtime",
      }),
    );
...
it("resolves relay client tracing from runtime config with build-time fallback", () => {
  ...
  assert.deepEqual(
    resolveRelayClientTracingConfig(
      {
        PIKU_RELAY_CLIENT_OTLP_TRACES_URL: "https://runtime.example.test/v1/traces",
        PIKU_RELAY_CLIENT_OTLP_TRACES_DATASET: "runtime-dataset",
        PIKU_RELAY_CLIENT_OTLP_TRACES_TOKEN: "runtime-token",
      },
      fallback,
    ),
    {
  ...
  assert.equal(
    resolveRelayClientTracingConfig(
      {
        PIKU_RELAY_CLIENT_OTLP_TRACES_URL: "http://insecure.example.test/v1/traces",
        PIKU_RELAY_CLIENT_OTLP_TRACES_DATASET: "runtime-dataset",
        PIKU_RELAY_CLIENT_OTLP_TRACES_TOKEN: "runtime-token",
      },
      fallback,
