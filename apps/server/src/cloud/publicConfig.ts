import { CONNECT_OAUTH_SCOPES, DEFAULT_HOSTED_APP_URL } from "@piku/shared/connectAuth";
import { clerkFrontendApiUrlFromPublishableKey } from "@piku/shared/relayAuth";
import { normalizeSecureRelayUrl } from "@piku/shared/relayUrl";
...
declare const __PIKU_BUILD_RELAY_URL__: string | undefined;
declare const __PIKU_BUILD_CLERK_PUBLISHABLE_KEY__: string | undefined;
declare const __PIKU_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__: string | undefined;
declare const __PIKU_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__: string | undefined;
declare const __PIKU_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__: string | undefined;
declare const __PIKU_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__: string | undefined;
...
export const buildTimeRelayUrl =
  typeof __PIKU_BUILD_RELAY_URL__ === "undefined"
    ? ""
    : (normalizeSecureRelayUrl(__PIKU_BUILD_RELAY_URL__) ?? "");
export const buildTimeClerkPublishableKey = readBuildTimeValue(
  typeof __PIKU_BUILD_CLERK_PUBLISHABLE_KEY__ === "undefined"
    ? undefined
    : __PIKU_BUILD_CLERK_PUBLISHABLE_KEY__,
);
export const buildTimeClerkCliOAuthClientId = readBuildTimeValue(
  typeof __PIKU_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__ === "undefined"
    ? undefined
    : __PIKU_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__,
);
export const buildTimeRelayClientTracing = {
  tracesUrl: readBuildTimeValue(
    typeof __PIKU_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__ === "undefined"
      ? undefined
      : __PIKU_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__,
  ),
  tracesDataset: readBuildTimeValue(
    typeof __PIKU_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__ === "undefined"
      ? undefined
      : __PIKU_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__,
  ),
  tracesToken: readBuildTimeValue(
    typeof __PIKU_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__ === "undefined"
      ? undefined
      : __PIKU_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__,
  ),
} as const;
...
export function resolveRelayClientTracingConfig(
  ...
  const tracesUrl = env.PIKU_RELAY_CLIENT_OTLP_TRACES_URL?.trim() || fallback.tracesUrl;
  const tracesDataset = env.PIKU_RELAY_CLIENT_OTLP_TRACES_DATASET?.trim() || fallback.tracesDataset;
  const tracesToken = env.PIKU_RELAY_CLIENT_OTLP_TRACES_TOKEN?.trim() || fallback.tracesToken;
...
export function makeRelayUrlConfig(fallback = buildTimeRelayUrl) {
  const runtimeConfig = Config.nonEmptyString("PIKU_RELAY_URL");
...
export const hasCloudPublicConfig = Boolean(
  (normalizeSecureRelayUrl(process.env.PIKU_RELAY_URL ?? "") ?? buildTimeRelayUrl) &&
  (process.env.PIKU_CLERK_PUBLISHABLE_KEY?.trim() || buildTimeClerkPublishableKey) &&
  (process.env.PIKU_CLERK_CLI_OAUTH_CLIENT_ID?.trim() || buildTimeClerkCliOAuthClientId),
);
