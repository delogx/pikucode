...
import type { DesktopHostTelemetrySnapshot, ResourceMonitorSnapshotEvent } from "@piku/contracts";
  ...
  it.effect("rejects Electron processes as signal targets", () =>
    Effect.gen(function* () {
      ...
      expect(result).toEqual({
        ...
        signal: "SIGKILL",
        signaled: false,
        message: Option.some("Process 4242 is not a signalable Piku backend descendant."),
      });
