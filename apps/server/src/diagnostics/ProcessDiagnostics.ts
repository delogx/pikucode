...
>()("piku/diagnostics/ProcessDiagnostics") {}
    ...
    function* (input) {
      if (input.pid === process.pid) {
        return {
          ...
          signal: input.signal,
          signaled: false,
          message: Option.some("Refusing to signal the Piku server process."),
        };
      }
      ...
      if (!canSignalCategory(selected.category)) {
        return {
          ...
          signal: input.signal,
          signaled: false,
          message: Option.some(`Process ${input.pid} is not a signalable Piku backend descendant.`),
        };
      }
