...
> {
  return Effect.gen(function* () {
    const remoteDir = yield* makeTempDir("pikucode-git-remote-");
...
const GitManagerTestLayer = GitVcsDriver.layer.pipe(
  Layer.provide(ServerConfig.layerTest(process.cwd(), { prefix: "piku-git-manager-test-" })),
  Layer.provideMerge(VcsProcess.layer),
  Layer.provideMerge(NodeServices.layer),
);
...
it.layer(GitManagerTestLayer)("GitManager", (it) => {
  it.effect("status includes PR metadata when branch already has an open PR", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("pikucode-git-manager-");
  ...
  it.effect("status returns an explicit non-repo result for non-git directories", () =>
    Effect.gen(function* () {
      const cwd = yield* makeTempDir("pikucode-git-manager-non-repo-");
      const { manager } = yield* makeManager();
  ...
  it.effect("status returns an explicit non-repo result for deleted directories", () =>
    Effect.gen(function* () {
      const rootDir = yield* makeTempDir("pikucode-git-manager-missing-dir-");
              ...
              JSON.stringify([
                {
                  ...
                  url: "https://github.com/pingdotgg/pikucode/pull/1661",
                  baseRefName: "main",
                  headRefName: "main",
                  state: "OPEN",
                  ...
                  headRepository: {
                    nameWithOwner: "lnieuwenhuis/pikucode",
                  },
                  headRepositoryOwner: {
                    login: "lnieuwenhuis",
                  },
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* runGit(repoDir, ["checkout", "-b", "pikucode/pr-488/statemachine"]);
        ...
        const status = yield* manager.status({ cwd: repoDir });
        expect(status.refName).toBe("pikucode/pr-488/statemachine");
        expect(status.pr).toEqual({
          number: 488,
          title: "Rebase this PR on latest main",
          url: "https://github.com/pingdotgg/codething-mvp/pull/488",
              ...
              "effect-atom": JSON.stringify([
                {
                  number: 1618,
                  title: "Correct PR",
                  url: "https://github.com/pingdotgg/pikucode/pull/1618",
                  baseRefName: "main",
              ...
              "upstream/effect-atom": JSON.stringify([
                {
                  number: 1518,
                  title: "Wrong PR",
                  url: "https://github.com/pingdotgg/pikucode/pull/1518",
                  baseRefName: "main",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* runGit(repoDir, ["remote", "add", "fork-seed", forkDir]);
        yield* runGit(repoDir, ["push", "-u", "fork-seed", "statemachine"]);
        yield* runGit(repoDir, ["checkout", "-b", "pikucode/pr-142/statemachine"]);
        yield* runGit(repoDir, ["branch", "--set-upstream-to", "fork-seed/statemachine"]);
        yield* configureVisibleRemoteUrlWithLocalRewrite(
          repoDir,
          "fork-seed",
        ...
        const { manager, ghCalls } = yield* makeManager({
          ghScenario: {
            prListByHeadSelector: {
              // @effect-diagnostics-next-line preferSchemaOverJson:off
              "pikucode/pr-142/statemachine": JSON.stringify([]),
              // @effect-diagnostics-next-line preferSchemaOverJson:off
  ...
  it.effect("accepts fork PR metadata when origin is the fork checkout remote", () =>
    Effect.sync(() => {
      const headContext = {
        headBranch: "pikucode/git-audit-stability",
        headRepositoryNameWithOwner: "justsomelegs/pikucode",
        ...
        GitManager.matchesBranchHeadContext(
          {
            ...
            url: "https://github.com/pingdotgg/pikucode/pull/2284",
            baseRefName: "main",
            headRefName: "pikucode/git-audit-stability",
            state: "open",
            updatedAt: Option.none(),
            isCrossRepository: true,
            headRepositoryNameWithOwner: "justsomelegs/pikucode",
  ...
  it.effect("generates PR content against the remote base when the local base is stale", () =>
    Effect.gen(function* () {
      ...
      const peerDir = yield* makeTempDir("pikucode-git-peer-");
  ...
  it.effect("creates cross-repo PRs with the fork owner selector and default base branch", () =>
    Effect.gen(function* () {
      ...
      yield* runGit(repoDir, ["commit", "-m", "Feature commit"]);
      yield* runGit(repoDir, ["push", "-u", "fork-seed", "statemachine"]);
      yield* runGit(repoDir, ["checkout", "-b", "pikucode/pr-91/statemachine"]);
      yield* runGit(repoDir, ["branch", "--set-upstream-to", "fork-seed/statemachine"]);
      yield* configureVisibleRemoteUrlWithLocalRewrite(
        repoDir,
        "fork-seed",
  ...
  it.effect("preserves both branch materialization failures when the fallback also fails", () =>
    Effect.gen(function* () {
      ...
      expect(error).toMatchObject({
        ...
        localBranch: "pikucode/pr-93/feature/missing-fork-branch",
      });
      ...
      const { manager } = yield* makeManager({
        ghScenario: {
          pullRequest: {
            ...
            url: "https://github.com/pingdotgg/pikucode/pull/642",
            baseRefName: "main",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(result.branch).toBe("pikucode/pr-91/main");
        expect(result.worktreePath).not.toBeNull();
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(result.branch).toBe("pikucode/pr-92/main");
