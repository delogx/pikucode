...
export const OrchestratorCapabilitiesTool = Tool.make("orchestrator_capabilities", {
  description:
    "List the V2 provider instances, models, inherited runtime settings, and app-owned orchestration features available to this Piku thread.",
...
export const DelegateTaskTool = Tool.make("delegate_task", {
  description:
    "Delegate one task to a Piku-owned child agent/subagent of THIS thread and run it with only the supplied task prompt, without copying parent conversation history. Use this whenever the user asks for an agent, subagent, worker, delegated task, or parallel help—including cross-provider work. The childThreadId is backing storage, not an ordinary top-level thread. Provider, model, model options (see orchestrator_capabilities), runtime mode, and interaction mode inherit unless target overrides them. Prefer mode='async' for long work; mode='wait' blocks until completion or timeout. An async child's completion wakes this thread with a continuation message naming the task (queued behind any turn in progress), so end the turn instead of polling or spawning watchers; use task_status only when the result is needed mid-turn.",
...
export const TaskStatusTool = Tool.make("task_status", {
  description:
    "Read the latest durable state and final summary for a Piku-owned delegated task created by this parent thread.",
...
export const TaskCancelTool = Tool.make("task_cancel", {
  description:
    "Request interruption of an active Piku-owned delegated task. Completed tasks are returned unchanged.",
...
export const CreateThreadsTool = Tool.make("create_threads", {
  ...
  .annotate(Tool.Title, "Create Piku threads")
  .annotate(Tool.Destructive, true)
  .annotate(Tool.OpenWorld, true);

export const ThreadStartTool = Tool.make("piku_thread_start", {
  description:
    "Create an ordinary TOP-LEVEL Piku conversation and immediately start its first turn. This is not a child agent/subagent; use delegate_task for delegated work. The new thread inherits this thread's project, checkout, provider, model, and runtime settings unless overridden. Use piku_thread_wait and piku_thread_read to collect its result.",
  ...
  .annotate(Tool.Title, "Start a Piku thread")
  .annotate(Tool.Destructive, true)
  .annotate(Tool.OpenWorld, true);

export const ThreadListTool = Tool.make("piku_thread_list", {
  description:
    "List Piku threads in the calling thread's project, newest first. Filter by durable run status or title and paginate with the returned cursor. Threads from other projects are never exposed.",
  ...
  .annotate(Tool.Title, "List Piku threads")
  .annotate(Tool.Readonly, true)
  .annotate(Tool.Destructive, false)
  .annotate(Tool.Idempotent, true);

export const ThreadReadTool = Tool.make("piku_thread_read", {
  description:
    "Read durable state and a paginated timeline from a Piku thread in the calling project. The default messages view returns user messages, assistant messages, and proposed plans; activity returns all summarized timeline items. Continue with afterPosition=nextPosition.",
  ...
  .annotate(Tool.Title, "Read a Piku thread")
  .annotate(Tool.Readonly, true)
  .annotate(Tool.Destructive, false)
  .annotate(Tool.Idempotent, true);

export const ThreadSendTool = Tool.make("piku_thread_send", {
  description:
    "Send a message to a Piku thread in the calling project. mode='auto' starts an idle thread, steers a fully active turn, or queues behind a turn that is not yet steerable. Use queue for a separate follow-up turn, steer for an in-flight update, or restart to interrupt-and-restart the active turn. clientRequestId makes retries idempotent.",
  ...
  .annotate(Tool.Title, "Send to a Piku thread")
  .annotate(Tool.Destructive, true)
  .annotate(Tool.OpenWorld, true);

export const ThreadWaitTool = Tool.make("piku_thread_wait", {
  description:
    "Wait for a Piku thread run to reach a terminal durable state. Without runId, the latest run at call time is selected; an idle thread returns immediately. Timeout does not interrupt work, so call again or use piku_thread_read/list after timedOut=true.",
  ...
  .annotate(Tool.Title, "Wait for a Piku thread")
  .annotate(Tool.Readonly, true)
  .annotate(Tool.Destructive, false)
  .annotate(Tool.Idempotent, true);

export const ThreadInterruptTool = Tool.make("piku_thread_interrupt", {
  description:
    "Request interruption of a running turn in a Piku thread in the calling project. Without runId, the newest interruptible run is selected. Terminal runs and threads without an active turn return without another side effect. clientRequestId makes retries idempotent.",
  ...
  .annotate(Tool.Title, "Interrupt a Piku thread")
  .annotate(Tool.Destructive, true);
