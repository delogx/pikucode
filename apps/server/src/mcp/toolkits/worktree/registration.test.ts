...
import { ProviderInstanceId, ThreadId } from "@piku/contracts";
...
it.effect("production mcp layer lists worktree tools over http", () =>
  Effect.scoped(
    Effect.gen(function* () {
      ...
      expect(toolNames).toContain("piku_worktree_handoff");
      expect(toolNames).toContain("piku_worktree_status");
      ...
      const handoff = tools.find((tool) => tool.name === "piku_worktree_handoff");
      ...
      const status = tools.find((tool) => tool.name === "piku_worktree_status");
