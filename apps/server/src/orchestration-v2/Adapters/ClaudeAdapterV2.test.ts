...
import type { ProviderModelChangeRequest } from "../ProviderModelChangeRequests.ts";
import {
...
describe("ClaudeAdapterV2 MCP query overrides", () => {
  ...
  it("pre-approves all piku-code tools when attaching an MCP session without an allowlist", () => {
    ...
    withMcpSession(threadId, () => {
      ...
      assert.deepEqual(overrides, {
        allowedTools: [CLAUDE_PIKU_MCP_TOOL_WILDCARD],
        mcpServers: PIKU_MCP_SERVERS,
      });
    });
  });

  it("extends an explicit allowlist with the piku-code wildcard", () => {
    const threadId = ThreadId.make("thread-claude-mcp-with-allowlist");
    withMcpSession(threadId, () => {
      const overrides = claudeMcpQueryOverrides({
        threadId,
        readOnlySandbox: false,
        allowedTools: ["Read", "mcp__piku-code__*"],
      });
  ...
  it("pre-approves only read-only piku-code tools in a read-only sandbox", () => {
    ...
    withMcpSession(threadId, () => {
      ...
      assert.isFalse(overrides.allowedTools?.includes(CLAUDE_PIKU_MCP_TOOL_WILDCARD));
    });
  });

  it("pre-approves only read-only piku-code tools in a read-only sandbox without an allowlist", () => {
    ...
    withMcpSession(threadId, () => {
      ...
      assert.deepEqual(overrides.allowedTools, [...CLAUDE_READ_ONLY_PIKU_MCP_ALLOWED_TOOLS]);
    });
  });
  ...
  it("matches the read-only allowlist to the orchestrator toolkit annotations", () => {
    const readOnlyToolNames = Object.values(OrchestratorToolkit.tools)
      ...
      .map((tool) => `mcp__piku-code__${tool.name}`)
      .sort();

    assert.deepEqual([...CLAUDE_READ_ONLY_PIKU_MCP_ALLOWED_TOOLS].sort(), readOnlyToolNames);
  });
});
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const attachmentsDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "piku-claude-v2-attachments-",
        });
  ...
  it.effect("rejects unsupported image types before opening a provider query", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const idAllocator = yield* IdAllocatorV2;
        const attachmentsDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "piku-claude-v2-unsupported-attachment-",
        });
        let openCount = 0;
        const adapter = makeClaudeAdapterV2({
          instanceId: CLAUDE_DEFAULT_INSTANCE_ID,
  ...
  it.effect("forks at the source assistant cursor and resumes the forked session", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const idAllocator = yield* IdAllocatorV2;
        const attachmentsDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "piku-claude-v2-fork-attachments-",
        });
  ...
  const openTurnWithOrdinal = (providerTurnOrdinal: number) =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const idAllocator = yield* IdAllocatorV2;
        const attachmentsDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "piku-claude-v2-session-identity-",
        });
        const openedQueries: Array<ClaudeAgentSdkQueryOpenInput> = [];
        const adapter = makeClaudeAdapterV2({
          instanceId: CLAUDE_DEFAULT_INSTANCE_ID,
