...
import { parseCliArgs } from "@piku/shared/cliArgs";
import { HostProcessEnvironment } from "@piku/shared/hostProcess";
import { applyClaudePromptEffortPrefix } from "@piku/shared/model";
import {
  type ChatAttachment,
  ClaudeSettings,
...
import {
  claudeModelSelectionForApiModelId,
...
>()("piku/orchestration-v2/Adapters/ClaudeAdapterV2/ClaudeAgentSdkQueryRunner") {}
    ...
    ...(input.mcpServers === undefined
      ...
      : {
          systemPrompt: {
            ...
            append: PIKU_CODE_ORCHESTRATION_INSTRUCTIONS,
          },
        }),
...
export const CLAUDE_PIKU_MCP_TOOL_WILDCARD = "mcp__piku-code__*";
...
export const CLAUDE_READ_ONLY_PIKU_MCP_ALLOWED_TOOLS: ReadonlyArray<string> = [
  "mcp__piku-code__orchestrator_capabilities",
  "mcp__piku-code__task_status",
  "mcp__piku-code__list_scheduled_tasks",
  "mcp__piku-code__piku_thread_list",
  "mcp__piku-code__piku_thread_read",
  "mcp__piku-code__piku_thread_wait",
];

// The SDK's `allowedTools` only pre-approves tool calls; availability is the
// separate `tools` option. Attaching the piku-code MCP server therefore always
...
export function claudeMcpQueryOverrides(input: {
  ...
  const mcpAllowedTools = input.readOnlySandbox
    ? CLAUDE_READ_ONLY_PIKU_MCP_ALLOWED_TOOLS
    : [CLAUDE_PIKU_MCP_TOOL_WILDCARD];
  return {
...
> {
  if (message.subtype === "success") {
    // A refusal that reaches the result means no fallback ran (the model has
    // no fallback target, the target is blocked, or switchModelsOnFlag asked
    // for a prompt the SDK surface cannot show); the turn produced no work.
  ...
  if (message.stop_reason === "refusal") {
    ...
    return makeProviderFailure({
      message:
        resultText.length > 0
          ? resultText
          : "Claude's safety classifiers declined this request and no fallback model ran. Edit the message and retry, or switch models and resend.",
      code: "refusal",
      class: "provider_error",
      retryable: false,
    });
...
export interface ClaudeAdapterV2Options {
  ...
  /** Sink for provider-initiated model changes; defaults to dropping them. */
  readonly modelChangeRequests?: {
...
export function makeClaudeAdapterV2(
  ...
  const modelChangeRequests = adapterOptions.modelChangeRequests ?? {
    offer: () => Effect.void,
  };
      ...
      function* (input: ProviderAdapterV2OpenSessionInput) {
            ...
            turnItem:
              nextWorkflow === null
                ...
                : {
                    ...
                    ...(nextWorkflow.script === null ? {} : { script: nextWorkflow.script }),
                    phases: nextWorkflow.phases,
                    agents: nextWorkflow.agents,
                    ...(task.progress === undefined ? {} : { progress: task.progress }),
                    result: task.result,
                    ...(nextWorkflow.usage.totalTokens === undefined
                      ? {}
                      : { totalTokens: nextWorkflow.usage.totalTokens }),
                    ...(nextWorkflow.usage.toolUses === undefined
                      ? {}
                      : { toolUses: nextWorkflow.usage.toolUses }),
                    ...(nextWorkflow.usage.durationMs === undefined
                      ? {}
                      : { durationMs: nextWorkflow.usage.durationMs }),
                  },
          });
        ...
        const handleSdkMessage = Effect.fnUntraced(function* (input: {
          ...
          if (message.type === "system" && message.subtype === "model_refusal_fallback") {
            // Claude Code re-ran a safety-flagged request on its fallback
            // model and the session stays on that model (direction "retry").
            // Surface the switch in the timeline and record the new model on
            // the thread, so the projection stops claiming the refused model.
            const occurredAt = yield* DateTime.now;
            const nativeItemId = `model-fallback:${message.uuid}`;
            const itemOrdinal = yield* resolveItemOrdinal(context, nativeItemId);
            const category = message.api_refusal_category ?? null;
            const explanation = message.api_refusal_explanation ?? null;
            yield* emitProviderEvent({
              type: "turn_item.updated",
              driver: CLAUDE_PROVIDER,
              turnItem: {
            ...
            yield* modelChangeRequests.offer({
              threadId: context.input.threadId,
              providerThreadId: context.input.providerThread.id,
              driver: CLAUDE_PROVIDER,
              ...
              fromModel: message.original_model,
              toModel: message.fallback_model,
              modelSelection: claudeModelSelectionForApiModelId(
                context.input.modelSelection,
                message.fallback_model,
              ),
              reason: "refusal_fallback",
            });
            yield* Effect.logInfo("orchestration-v2.claude-model-refusal-fallback", {
              providerTurnId: context.providerTurnId,
          ...
          // Piku did not mark as a continuation. That is the hang fix working,
...
> = {
  ...
  create: Effect.fn("ClaudeAdapterV2Driver.create")(
    function* (input: ProviderAdapterDriverCreateInput<ClaudeSettings>) {
      ...
      const modelChangeRequests = yield* ProviderModelChangeRequests;
