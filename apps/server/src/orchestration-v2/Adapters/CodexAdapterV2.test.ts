...
describe("CodexAdapterV2 runtime policy", () => {
  it.effect("derives concrete Codex turn policies from every Piku runtime mode", () =>
    Effect.gen(function* () {
      const build = (
  ...
  it.effect("adds default-mode developer instructions when the Piku MCP server is attached", () =>
    Effect.gen(function* () {
      const params = yield* buildCodexTurnStartParams({
  ...
  it.effect("omits default-mode collaboration settings without the Piku MCP server", () =>
    Effect.gen(function* () {
      const params = yield* buildCodexTurnStartParams({
        nativeThreadId: "native-default-without-piku-mcp",
  ...
  it.effect("adds Piku plan-mode developer instructions when the Piku MCP server is attached", () =>
    Effect.gen(function* () {
      const params = yield* buildCodexTurnStartParams({
        nativeThreadId: "native-plan-with-piku-mcp",
  ...
  it.effect("keeps Codex in plan mode without referencing unavailable Piku MCP tools", () =>
    Effect.gen(function* () {
      const params = yield* buildCodexTurnStartParams({
        nativeThreadId: "native-plan-without-piku-mcp",
        codexInput: [{ type: "text", text: "plan this task" }],
        runtimePolicy: {
          runtimeMode: "full-access",
          interactionMode: "plan",
...
describe("CodexAdapterV2 dynamic tool projection", () => {
  it("preserves MCP arguments and prefers structured output", () => {
    ...
    assert.deepEqual(projection, {
      toolName: "piku-code.create_threads",
      input: {
        threads: [{ title: "Fixture child", prompt: "fixture child prompt" }],
      },
      output: {
  ...
  return [
    {
      ...
      frame: {
        ...
        method: "initialize",
        params: {
          clientInfo: { name: "pikucode_desktop", title: "Piku Code Desktop", version: "0.1.0" },
    ...
    },
    {
      ...
      frame: {
        id: 1,
        result: {
          userAgent: "pikucode_desktop/0.144.0",
