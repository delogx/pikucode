...
const orchestratorProvided = orchestratorLayer.pipe(
  Layer.provide(
    Layer.mergeAll(
      ...
      idAllocatorLayer,
      providerAdapterRegistryProvided,
      // Same layer references as their workers and the adapter
...
const providerModelChangeWorkerProvided = providerModelChangeWorkerLive.pipe(
  Layer.provide(Layer.merge(providerModelChangeRequestsLayer, threadManagementProvided)),
);
...
export const OrchestrationV2ProductionLayerLive = Layer.mergeAll(
  ...
  providerModelChangeWorkerProvided,
);
