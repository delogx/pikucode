...
const makeCheckpointWorkspace = Effect.gen(function* () {
  ...
  const cwd = yield* fs.makeTempDirectory({ prefix: "piku-orchestrator-v2-thread-fork-" });
  yield* runGit(cwd, ["init"]);
  yield* runGit(cwd, ["config", "user.name", "Piku Code Test"]);
  yield* runGit(cwd, ["config", "user.email", "pikucode-test@example.com"]);
