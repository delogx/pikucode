...
const TestLayer = Layer.empty.pipe(
  Layer.provideMerge(PikuProjectFileLoader.layer),
...
const writeProjectFile = Effect.fn("writeProjectFile")(function* (cwd: string, contents: string) {
  ...
  yield* fileSystem.writeFileString(path.join(cwd, "piku.json"), contents).pipe(Effect.orDie);
});

it.layer(TestLayer)("PikuProjectFileLoader", (it) => {
  describe("load", () => {
    it.effect("loads and decodes a valid piku.json", () =>
      Effect.gen(function* () {
        const loader = yield* PikuProjectFileLoader.PikuProjectFileLoader;
    ...
    it.effect("returns none when piku.json is missing", () =>
      Effect.gen(function* () {
        const loader = yield* PikuProjectFileLoader.PikuProjectFileLoader;
        const cwd = yield* makeTempDir;
