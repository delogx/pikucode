...
import { CommandId, type Project, ProjectId, ProviderInstanceId } from "@piku/contracts";
...
const metadataLayer = Layer.merge(
  Layer.succeed(RepositoryIdentityResolver.RepositoryIdentityResolver, {
    resolve: (workspaceRoot) =>
      Effect.succeed({
        canonicalKey: `github.com/piku/${workspaceRoot.split("/").at(-1)}`,
        locator: {
          source: "git-remote" as const,
          remoteName: "origin",
          remoteUrl: `git@github.com:piku/${workspaceRoot.split("/").at(-1)}.git`,
        },
...
it.layer(TestLayer)("ProjectService", (it) => {
  it.effect("creates, updates, resolves, snapshots, and soft-deletes projects", () =>
    Effect.gen(function* () {
      ...
      assert.equal(hydratedCreated?.repositoryIdentity?.canonicalKey, "github.com/piku/project");
  ...
  () =>
    Effect.gen(function* () {
        ...
        Layer.succeed(RepositoryIdentityResolver.RepositoryIdentityResolver, {
          resolve: (workspaceRoot) =>
            Effect.gen(function* () {
              ...
              return {
                canonicalKey: "github.com/piku/slow-project",
                locator: {
                  source: "git-remote" as const,
                  remoteName: "origin",
                  remoteUrl: "git@github.com:piku/slow-project.git",
                },
                rootPath: workspaceRoot,
              };
            }),
      ...
      yield* Effect.gen(function* () {
        ...
        assert.equal(firstProject.repositoryIdentity?.canonicalKey, "github.com/piku/slow-project");
        ...
        assert.equal(byId.repositoryIdentity?.canonicalKey, "github.com/piku/slow-project");
