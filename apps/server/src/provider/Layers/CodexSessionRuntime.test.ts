...
import { DEFAULT_MODEL, ThreadId } from "@piku/contracts";
...
describe("buildCodexDeveloperInstructions", () => {
  it("appends runtime info after the mode instructions", () => {
    ...
    NodeAssert.match(instructions, /Piku Code/);
...
describe("Piku browser developer instructions", () => {
...
describe("hasConfiguredMcpServer", () => {
  it("detects inline Codex MCP configuration arguments", () => {
    ...
    NodeAssert.equal(
      hasConfiguredMcpServer(["-c", 'mcp_servers.piku-code.url="http://127.0.0.1/mcp"']),
      true,
    );
  });
});
  ...
  it("keeps launch args when explicit app-server args are provided", () => {
    NodeAssert.deepStrictEqual(
      codexSessionAppServerArgs(
        ["-c", "mcp_servers.piku-code.url=http://127.0.0.1/mcp"],
      ...
      [
        ...
        "foo",
        "-c",
        "mcp_servers.piku-code.url=http://127.0.0.1/mcp",
      ],
    );
  });
});
