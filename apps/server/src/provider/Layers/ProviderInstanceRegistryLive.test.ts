 ...
 * without trying to spawn real `codex` / `claude` binaries. That keeps the assertions focused on registry routing
...
describe("ProviderInstanceRegistryLive — all drivers slice", () => {
  ...
  // Path). `NodeServices.layer` provides `ServerConfig.layerTest`'s
  ...
  it.live("boots one instance of every shipped driver from a single config map", () =>
    Effect.gen(function* () {
      ...
      // trait (they must still be different object values).
      const adapters = [codex!.orchestrationAdapter, claude!.orchestrationAdapter];
