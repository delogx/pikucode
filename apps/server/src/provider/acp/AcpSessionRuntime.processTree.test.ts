...
describe("terminatePosixOwnedProcessTree", () => {
  ...
  it.live("sweeps empty piku-acp sibling leases owned by dead pids only", () =>
    Effect.gen(function* () {
      ...
      const staleEmpty = writeSibling(`piku-acp-${deadPid}-aaaa`, "0");
      const currentPidSibling = writeSibling(`piku-acp-${process.pid}-bbbb`, "0");
      const liveForeignSibling = writeSibling(`piku-acp-${liveForeignPid}-cccc`, "0");
      const stalePopulated = writeSibling(`piku-acp-${deadPid}-dddd`, "1");
      ...
      try {
        ...
        writeSibling(`piku-acp-${deadPid}-eeee`, null);
        sweepStaleLinuxCgroupSiblings(parent, {
          isProcessAlive,
          readPopulated: (path) => populatedByPath.get(path),
          remove: (path) => {
        ...
        expect(NodeFS.existsSync(NodePath.join(parent, `piku-acp-${deadPid}-eeee`))).toBe(true);
        expect(NodeFS.existsSync(currentPidSibling)).toBe(true);
      } finally {
  ...
  it.live("preserves exact argv and strips wrapper-only environment before exec", () =>
    Effect.gen(function* () {
      ...
      const linkedNode = NodePath.join(scratch, "Piku Code AppImage 'quoted'");
      ...
      const target = [
        ...
        "  wrapper: process.env.PIKU_ACP_CGROUP_WRAPPER,",
        "}));",
      ].join("\n");
  ...
  it.live("kills a post-TERM detached double fork without touching an unrelated sentinel", () =>
    Effect.gen(function* () {
      ...
      const provider = NodeChildProcess.spawn(wrapped.command, wrapped.args, {
        detached: true,
        env: { ...process.env, ELECTRON_RUN_AS_NODE: "1", PIKU_ACP_CGROUP_WRAPPER: "1" },
        stdio: "ignore",
      });
      provider.unref();
  ...
  it.live("re-kills repopulated cgroups and verifies root removal after retryable errors", () =>
    Effect.gen(function* () {
      ...
      const lease: AcpLinuxCgroupLease = {
        contains: () => false,
        exists: () => exists,
        path: "/test/piku-acp-repopulation",
        relativePath: "/test/piku-acp-repopulation",
        kill: () => {
          killCalls += 1;
          populated = false;
        },
  ...
  it.live("reports persistent retryable removal failures as removal failures", () =>
    Effect.gen(function* () {
      ...
      const lease: AcpLinuxCgroupLease = {
        contains: () => false,
        exists: () => true,
        path: "/test/piku-acp-busy",
        relativePath: "/test/piku-acp-busy",
        kill: () => undefined,
        populated: () => false,
        remove: () => {
          throw removeError;
      ...
      expect(error.detail).toBe("Failed to remove ACP cgroup /test/piku-acp-busy");
  ...
  it("sends TERM only to the exact captured root while it remains in the child cgroup", () => {
    ...
    const lease: AcpLinuxCgroupLease = {
      contains: () => true,
      exists: () => true,
      path: "/test/piku-acp-root",
      relativePath: "/test/piku-acp-root",
      kill: () => undefined,
      populated: () => true,
      remove: () => undefined,
    };
  ...
  it.live("re-admits a still-owned child after PID reuse and never signals the Piku session", () =>
    Effect.gen(function* () {
