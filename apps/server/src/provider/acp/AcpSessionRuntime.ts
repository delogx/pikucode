  ...
  return {
    ...
    args: [
      ...
      [
        ...
        "  delete env.PIKU_ACP_CGROUP_WRAPPER;",
...
const STALE_ACP_CGROUP_SIBLING = /^piku-acp-(\d+)-/;
...
/** Best-effort removal of empty `piku-acp-<dead-pid>-*` sibling leases under a parent cgroup. */
...
function tryCreateLinuxCgroupLease(
  ...
  const childName = `piku-acp-${process.pid}-${NodeCrypto.randomUUID().replaceAll("-", "")}`;
...
export function terminatePosixOwnedProcessTree(input: {
  ...
  const signalPhase = (signal: NodeJS.Signals, includeRoot: boolean) => {
    ...
    if (current === undefined) throw fail("Cannot identify the current Piku process group");
    ...
    const adoptSession = (
      ...
      Effect.gen(function* () {
        ...
        const syntheticReplayIdle =
          ...
          typeof meta === "object" &&
          !Array.isArray(meta) &&
          (meta as { readonly pikuSessionLoadReady?: unknown }).pikuSessionLoadReady ===
            "replay_idle";
