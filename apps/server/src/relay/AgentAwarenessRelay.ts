...
import { projectThreadAwarenessV2 } from "@piku/shared/agentAwareness";
import { makeDrainableWorker } from "@piku/shared/DrainableWorker";
...
>()("piku/relay/AgentAwarenessRelay") {}
...
const makePublishProof = Effect.fn("makePublishProof")(function* (input: {
  ...
  const payload = {
    iss: `piku-env:${input.environmentId}`,
      ...
      switch (startupState) {
        case "waiting-for-link":
          yield* Effect.logInfo(
            "agent activity publishing standby; waiting for Piku Connect link reconciliation",
          );
          break;
        case "disabled":
          yield* Effect.logInfo("agent activity publishing disabled by Piku Connect configuration");
          break;
        case "enabled":
