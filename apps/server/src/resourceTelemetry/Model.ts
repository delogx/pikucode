...
export interface TelemetryCounters {
  ...
  readonly allPiku: GroupCounters;
}
...
export interface MergeProcessesResult {
  ...
  readonly groups: {
    ...
    readonly allPiku: ResourceTelemetryAggregate;
  };
...
function applyLifecycleCounters(input: {
  ...
  let allPiku = input.counters.allPiku;
  for (const processDelta of input.deltas) {
    ...
    allPiku = incrementCounters(allPiku, processDelta);
  }
  ...
  for (const [identityKey, current] of input.current) {
    ...
    allPiku = incrementCounters(allPiku, { processStarts: 1 });
  }
  ...
  for (const [identityKey, previous] of input.previous) {
    ...
    allPiku = incrementCounters(allPiku, { processExits: 1 });
  }

  return { backend, electron, monitor, allPiku };
}
...
export function mergeProcesses(input: MergeProcessesInput): MergeProcessesResult {
  ...
  return {
    ...
    groups: {
      ...
      allPiku: aggregate(ordered, counters.allPiku),
    },
    deltas: processDeltas,
  };
}
