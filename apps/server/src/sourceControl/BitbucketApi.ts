...
import { sanitizeBranchFragment } from "@piku/shared/git";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("PIKU_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("PIKU_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("PIKU_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("PIKU_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("piku/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `pikucode/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set PIKU_BITBUCKET_EMAIL and PIKU_BITBUCKET_API_TOKEN, or PIKU_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
