...
import { CodexSettings, ProviderInstanceId, TextGenerationError } from "@piku/contracts";
...
const CodexTextGenerationTestLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
  prefix: "pikucode-codex-text-generation-test-",
  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "  cat > \"$output_path\" <<'__PIKU_FAKE_CODEX_OUTPUT__'",
        input.output,
        "__PIKU_FAKE_CODEX_OUTPUT__",
        "fi",
...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "pikucode-codex-text-" });
...
it.layer(CodexTextGenerationTestLayer)("CodexTextGeneration", (it) => {
  ...
  it.effect("uses PIKU_CODEX_LAUNCH_ARGS for codex exec over settings", () =>
    withFakeCodexEnv(
      {
        output: JSON.stringify({
          subject: "Add important change",
        ...
        environment: { PIKU_CODEX_LAUNCH_ARGS: " --strict-config --listen off " },
        requireArg: "--strict-config",
