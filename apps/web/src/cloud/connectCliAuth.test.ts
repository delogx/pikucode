...
describe("connectCliAuth", () => {
  ...
  it("requires both the publishable key and the CLI OAuth client id", () => {
    ...
    vi.stubEnv("VITE_CLERK_JWT_TEMPLATE", "piku-relay");
    vi.stubEnv("VITE_PIKU_RELAY_URL", "https://relay.example.com");
  ...
  it("builds the Clerk authorize URL with the configured hosted origin's callback", () => {
    vi.stubEnv("VITE_CLERK_PUBLISHABLE_KEY", TEST_PUBLISHABLE_KEY);
    vi.stubEnv("VITE_CLERK_CLI_OAUTH_CLIENT_ID", "oauthapp_123");
    vi.stubEnv("VITE_HOSTED_APP_URL", "https://nightly.app.pikucode.dev");
    ...
    expect(url.searchParams.get("redirect_uri")).toBe(
      "https://nightly.app.pikucode.dev/connect/callback",
    );
  ...
  it("reads the code and state Clerk echoes back to the callback", () => {
    expect(
      readConnectCliCallbackResult(
        new URL("https://app.pikucode.dev/connect/callback?code=abc&state=state-1"),
      ),
    ).toEqual({ code: "abc", state: "state-1" });
    expect(
      readConnectCliCallbackResult(new URL("https://app.pikucode.dev/connect/callback?code=abc")),
    ).toBeNull();
    expect(
      readConnectCliCallbackResult(new URL("https://app.pikucode.dev/connect/callback?state=s")),
    ).toBeNull();
  });
});
