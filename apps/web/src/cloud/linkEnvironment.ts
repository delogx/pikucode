...
import { EnvironmentRegistry } from "@piku/client-runtime/connection";
import { request, runStream } from "@piku/client-runtime/rpc";
import { makeEnvironmentHttpApiClient } from "@piku/client-runtime/rpc";
import { ManagedRelay } from "@piku/client-runtime/relay";
  ...
  return Effect.gen(function* () {
    ...
    if (status.status === "unsupported") {
      return yield* new CloudEnvironmentLinkError({
        message: `Piku Code cannot install the relay client automatically on ${status.platform}-${status.arch}.`,
      });
    }
      ...
      return yield* new CloudEnvironmentLinkError({
        message:
          installedStatus.status === "unsupported"
            ? `Piku Code cannot install the relay client automatically on ${installedStatus.platform}-${installedStatus.arch}.`
...
> {
  return Effect.gen(function* () {
    ...
    if (!configuredRelayUrl) {
      return yield* new CloudEnvironmentLinkError({
        message: "PIKU_RELAY_URL is not configured.",
      });
    }
...
// independently of Piku Connect while clients reach the environment out of band.
