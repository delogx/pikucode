...
/**
 * Drives the primary environment's Piku Connect link. Piku Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message =
      cause instanceof Error ? cause.message : "Could not update Piku Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[piku-connect] Could not update Piku Connect", { message, traceId, cause });
    setOperationError(traceId ? `${message} Trace ID: ${traceId}` : message);
    toastManager.add({
      type: "error",
      title: "Could not update Piku Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn Piku Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Piku Connect before enabling this."));
        return false;
      }
