import { EnvironmentId, type VcsRef } from "@piku/contracts";
...
describe("resolvePreviousWorktreeSeed", () => {
  it("picks the most recently updated worktree thread", () => {
    expect(
      resolvePreviousWorktreeSeed({
        threads: [
          {
            branch: "piku/older",
            worktreePath: "/repo/.pikucode/worktrees/older",
            updatedAt: "2026-07-20T00:00:00.000Z",
          },
          {
            branch: "piku/newer",
            worktreePath: "/repo/.pikucode/worktrees/newer",
    ...
    ).toEqual({ branch: "piku/newer", worktreePath: "/repo/.pikucode/worktrees/newer" });
  });
    ...
    expect(
      resolvePreviousWorktreeSeed({
        threads: [
          {
            branch: "piku/current",
            worktreePath: "/repo/.pikucode/worktrees/current",
            updatedAt: "2026-07-22T00:00:00.000Z",
          },
        ],
        currentWorktreePath: "/repo/.pikucode/worktrees/current",
      }),
    ).toBeNull();
  });
  ...
  it("ignores archived threads and threads with unparseable timestamps", () => {
    expect(
      resolvePreviousWorktreeSeed({
        threads: [
          {
            branch: "piku/archived",
            worktreePath: "/repo/.pikucode/worktrees/archived",
            updatedAt: "2026-07-23T00:00:00.000Z",
            archivedAt: "2026-07-23T01:00:00.000Z",
          },
          {
            branch: "piku/garbage-timestamp",
            worktreePath: "/repo/.pikucode/worktrees/garbage",
            updatedAt: "not-a-date",
          },
          {
            branch: "piku/live",
            worktreePath: "/repo/.pikucode/worktrees/live",
    ...
    ).toEqual({ branch: "piku/live", worktreePath: "/repo/.pikucode/worktrees/live" });
  });
});

describe("resolvePreviousWorktreeLabel", () => {
  it("includes the branch when known", () => {
    expect(resolvePreviousWorktreeLabel({ branch: "piku/fix-thing", worktreePath: "/wt" })).toBe(
      "Previous worktree (piku/fix-thing)",
    );
...
describe("resolveDraftEnvModeAfterBranchChange", () => {
  it("switches to local mode when returning from an existing worktree to the main worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        ...
        currentWorktreePath: "/repo/.pikucode/worktrees/feature-a",
  ...
  it("uses worktree mode when selecting a ref already attached to a worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.pikucode/worktrees/feature-a",
        currentWorktreePath: null,
...
describe("resolveLocalCheckoutBranchMismatch", () => {
  ...
  it("ignores dedicated worktrees because their checkout is already thread-scoped", () => {
    expect(
      resolveLocalCheckoutBranchMismatch({
        effectiveEnvMode: "worktree",
        activeWorktreePath: "/repo/.pikucode/worktrees/feature-thread",
        activeThreadBranch: "feature/thread",
        currentGitBranch: "feature/current",
      }),
    ).toBeNull();
...
describe("resolveEffectiveEnvMode", () => {
  it("treats draft threads already attached to a worktree as current-checkout mode", () => {
    expect(
      resolveEffectiveEnvMode({
        activeWorktreePath: "/repo/.pikucode/worktrees/feature-a",
...
describe("resolveCurrentWorkspaceLabel", () => {
  ...
  it("describes the active checkout as a worktree when one is attached", () => {
    expect(resolveCurrentWorkspaceLabel("/repo/.pikucode/worktrees/feature-a")).toBe(
      "Current worktree",
    );
  });
});
...
describe("resolveLockedWorkspaceLabel", () => {
  ...
  it("uses a shorter label for an attached worktree", () => {
    expect(resolveLockedWorkspaceLabel("/repo/.pikucode/worktrees/feature-a")).toBe("Worktree");
  });
});

describe("resolveWorkspaceDisplayName", () => {
  it("returns the final folder for POSIX and Windows paths", () => {
    expect(resolveWorkspaceDisplayName("/repo/.pikucode/worktrees/feature-a")).toBe("feature-a");
...
describe("resolveBranchSelectionTarget", () => {
  it("reuses an existing secondary worktree for the selected ref", () => {
    expect(
      resolveBranchSelectionTarget({
        ...
        refName: {
          isDefault: false,
          worktreePath: "/repo/.pikucode/worktrees/feature-b",
        },
      }),
    ).toEqual({
      checkoutCwd: "/repo/.pikucode/worktrees/feature-b",
      nextWorktreePath: "/repo/.pikucode/worktrees/feature-b",
  ...
  it("keeps checkout in the current worktree for non-default refs", () => {
    expect(
      ...
      checkoutCwd: "/repo/.pikucode/worktrees/feature-a",
      nextWorktreePath: "/repo/.pikucode/worktrees/feature-a",
      reuseExistingWorktree: false,
    });
  });
