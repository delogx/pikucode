...
import { effectiveSettled, effectiveSnoozed } from "@piku/client-runtime/state/thread-settled";
import {
  deriveThreadActivityRun,
  deriveLatestThreadRun,
  deriveThreadRuntime,
} from "@piku/client-runtime/state/thread-execution";
import { resolveThreadProviderSession } from "@piku/client-runtime/state/thread-workflows";
import { resolveThreadLastVisitedAt } from "./Sidebar.logic";
import { derivePendingThreadRequests } from "@piku/client-runtime/state/thread-requests";
import {
...
import { CHAT_LIST_ANCHOR_OFFSET } from "@piku/shared/chatList";
import { projectScriptCwd, projectScriptRuntimeEnv } from "@piku/shared/projectScripts";
import { truncate } from "@piku/shared/String";
import { nextTerminalId, resolveTerminalSessionLabel } from "@piku/shared/terminalLabels";
...
import { buildTemporaryWorktreeBranchName } from "@piku/shared/git";
...
function ChatViewContent(props: ChatViewProps) {
  ...
  // A provider can switch the session's model on its own — Claude Code
  // re-runs safety-flagged requests on its fallback model and keeps the
  // session there. The server records the switch on the thread; the
  // thread-scoped composer draft must follow, or the model picker keeps
  // showing the refused model the user originally selected.
  const lastServerModelSelectionRef = useRef<{
  ...
  useEffect(() => {
    ...
    lastServerModelSelectionRef.current = {
      threadKey,
    ...
    if (
      ...
      modelSelectionsEqual(previous.modelSelection, activeThread.modelSelection)
    ) {
      return;
    }
