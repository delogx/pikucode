import { CheckpointRef, EnvironmentId, MessageId, RunId, ThreadId } from "@piku/contracts";
...
describe("MessagesTimeline", () => {
  ...
  it("renders Piku MCP dynamic tools with the product logo and pretty name", async () => {
    const { MessagesTimeline } = await import("./MessagesTimeline");
    const item = {
      id: "tool-piku-thread-read",
      threadId: "thread-source",
      runId: null,
      nodeId: null,
      providerThreadId: null,
      ...
      updatedAt: {},
      type: "dynamic_tool",
      toolName: "mcp__piku-code__piku_thread_read",
    ...
    expect(markup).toContain('data-tool-logo="piku-code"');
    expect(markup).toContain('src="/apple-touch-icon.png"');
    expect(markup).toContain("Read a Piku thread");
    expect(markup).not.toContain("mcp__piku-code__piku_thread_read");
  });
        ...
        timelineEntries={[
          {
            ...
            entry: {
              ...
              toolLifecycleStatus: "completed",
              changedFiles: ["C:/Users/mike/dev-stuff/pikucode/apps/web/src/session-logic.ts"],
            },
          },
        ]}
        workspaceRoot="C:/Users/mike/dev-stuff/pikucode"
      />,
    );

    expect(markup).toContain("pikucode/apps/web/src/session-logic.ts");
    expect(markup).not.toContain("C:/Users/mike/dev-stuff/pikucode/apps/web/src/session-logic.ts");
  });
