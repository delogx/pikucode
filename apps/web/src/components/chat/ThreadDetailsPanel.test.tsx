import type { EnvironmentId, PikuProjectFileScript, ThreadId } from "@piku/contracts";
import { renderToStaticMarkup } from "react-dom/server";
import { beforeEach, describe, expect, it, vi } from "vite-plus/test";

const testState = vi.hoisted(() => ({
  usePikuProjectFileScripts: vi.fn(),
  projectScriptsControl: vi.fn(),
}));

vi.mock("../../hooks/usePikuProjectFileScripts", () => ({
  usePikuProjectFileScripts: (...args: ReadonlyArray<unknown>) =>
    testState.usePikuProjectFileScripts(...args),
}));
...
describe("ThreadDetailsPanel", () => {
  beforeEach(() => {
    testState.usePikuProjectFileScripts.mockReset();
    testState.projectScriptsControl.mockReset();
  });

  it("passes checked-in piku.json scripts to the project scripts control", () => {
    ...
    ] satisfies ReadonlyArray<PikuProjectFileScript>;
    testState.usePikuProjectFileScripts.mockReturnValue(fileScripts);
    ...
    expect(testState.usePikuProjectFileScripts).toHaveBeenCalledWith(environmentId, gitCwd);
