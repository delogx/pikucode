...
import type { TimestampFormat } from "@piku/contracts/settings";
import {
  ArrowRightIcon,
  BotIcon,
  ChevronDownIcon,
...
export function V2LifecycleRow(props: {
  ...
  if (item.type === "model_fallback") {
    // The provider switched the session's model itself (safety-classifier
    // refusal fallback), so the models are provider-native ids; resolve
    // display names through the run's provider instance like handoff does.
    const fallbackRun =
      item.runId === null ? undefined : props.runs.find((run) => run.id === item.runId);
    const instanceId = fallbackRun?.providerInstanceId;
    const modelEndpoint = (model: string) =>
      instanceId === undefined ? (
        <span className="max-w-40 truncate">{stripContextWindowSuffix(model)}</span>
      ) : (
        <HandoffEndpoint
          providers={props.providerStatuses}
      ...
      <TimelineSystemDivider
        ...
        detail={
          <span
            ...
            {...(item.explanation === null ? {} : { title: item.explanation })}
          >
            {modelEndpoint(item.fromModel)}
            <ArrowRightIcon aria-hidden="true" className="size-3 shrink-0" />
            {modelEndpoint(item.toModel)}
            <span className="truncate">
              {item.category === null
                ? "· request flagged by safety classifiers"
                : `· request flagged (${item.category})`}
            </span>
          </span>
        }
      />
...
/**
 * Provider-native model ids can carry a context-window suffix (Claude's
 * `claude-opus-5[1m]`); the bare id is the piku model slug used for display
 * lookups.
 */
