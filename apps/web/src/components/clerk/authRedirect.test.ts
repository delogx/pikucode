...
describe("resolveClerkSignInProps", () => {
  it("returns to the current browser URL on the web", () => {
    const href = "https://app.pikucode.dev/connect?state=state-1#details";
  ...
  it("omits the redirect override on packaged desktop", () => {
    expect(resolveClerkSignInProps("piku://app/#/settings/general", true)).toEqual({});
  });

  it("omits the redirect override on development desktop", () => {
    expect(resolveClerkSignInProps("piku-dev://app/#/settings/general", true)).toEqual({});
  });
});
