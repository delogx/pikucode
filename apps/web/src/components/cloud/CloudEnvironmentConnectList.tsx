import { findErrorTraceId } from "@piku/client-runtime/errors";
import {
...
import type { RelayClientEnvironmentRecord } from "@piku/contracts/relay";
...
/**
 * The user's Piku Connect environments from relay discovery, each with a
  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      toastManager.add({
        ...
        description: `Connecting to ${environment.label} through Piku Connect.`,
      });
      return;
    }
    ...
    const message =
      cause instanceof Error ? cause.message : "Could not connect the Piku Connect environment.";
    const traceId = findErrorTraceId(cause);
    console.error("[piku-connect] Could not connect environment", { message, traceId, cause });
    toastManager.add({
      type: "error",
      ...
      return (
        <div className={ITEM_ROW_CLASSNAME}>
          <p className="text-sm font-medium text-destructive">
            Could not load Piku Connect environments
          </p>
