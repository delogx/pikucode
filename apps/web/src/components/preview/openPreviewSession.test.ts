import type { PreviewOpenInput, PreviewSessionSnapshot, ScopedThreadRef } from "@piku/contracts";
...
describe("openPreviewSession", () => {
  ...
  it("applies the RPC response without waiting for a preview event", async () => {
    ...
    expect(open).toHaveBeenCalledWith({ threadId: "thread-1", url: "piku.chat" });
    expect(readThreadPreviewState(threadRef).snapshot).toEqual(snapshot);
    expect(readThreadPreviewState(threadRef).recentlySeenUrls).toEqual(["https://piku.chat/"]);
  });
