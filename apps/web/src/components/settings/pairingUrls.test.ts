...
describe("settings pairing URL helpers", () => {
  ...
  it("uses hosted pairing URLs for HTTPS endpoints", () => {
    ...
    expect(resolveHostedPairingUrl("https://host.example.test:3773", "PAIRCODE")).toBe(
      "https://preview.pikucode.dev/pair?host=https%3A%2F%2Fhost.example.test%3A3773#token=PAIRCODE",
    );
  });
});
