...
export function getElectronPlatformClassNames(): readonly [typeof ELECTRON_CLASS_NAME] {
  return [ELECTRON_CLASS_NAME];
}

export function syncDocumentElectronPlatformClasses(): () => void {
  if (typeof document === "undefined") {
    return () => {};
  }

  const classNames = getElectronPlatformClassNames();
