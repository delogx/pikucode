...
describe("Pierre file icons", () => {
  ...
  it("extends Pierre with Piku-specific exact filename icons", () => {
    assert.equal(
      resolvePierreIconForEntry("package.json", "file")?.name,
      "piku-file-icon-package-json",
    );
    assert.equal(
      resolvePierreIconForEntry("config/tsconfig.json", "file")?.name,
      "piku-file-icon-tsconfig",
    );
    assert.equal(resolvePierreIconForEntry("AGENTS.md", "file")?.name, "piku-file-icon-agents");
    assert.equal(resolvePierreIconForEntry("CLAUDE.md", "file")?.name, "piku-file-icon-claude");
    assert.equal(resolvePierreIconForEntry("README.md", "file")?.name, "piku-file-icon-readme");
    assert.equal(resolvePierreIconForEntry("pnpm-lock.yaml", "file")?.name, "piku-file-icon-pnpm");
    assert.equal(
  ...
  it("ships every custom icon referenced by the extended resolver", () => {
    const customIconNames = new Set(Object.values(PIKU_PIERRE_ICONS.byFileName));
    for (const iconName of customIconNames) {
      assert.include(PIKU_PIERRE_ICONS.spriteSheet, `id="${iconName}"`);
    }
  });
