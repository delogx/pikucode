import type { EnvironmentId, ServerConfig, ServerSelfUpdateCapability } from "@piku/contracts";
...
export const VERSION_MISMATCH_DISMISSALS_STORAGE_KEY = "pikucode:version-mismatch-dismissals:v1";
...
export function resolveVersionMismatch(
  ...
  return {
    ...
    hint: "Version mismatch. Try syncing the client and server to the same Piku Code version.",
  };
}
...
export function serverUpdateGuidance(
  ...
  switch (capability) {
    ...
    case "desktop-managed":
      return `The ${serverLabel} is run by the Piku Code desktop app on its machine — update the desktop app there to sync them.`;
    default:
