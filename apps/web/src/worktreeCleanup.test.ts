import { EnvironmentId, ProjectId, ProviderInstanceId, ThreadId } from "@piku/contracts";
...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.pikucode/worktrees/pikucode-mvp/pikucode-4e609bb8",
    );
    expect(result).toBe("pikucode-4e609bb8");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.pikucode\\worktrees\\pikucode-mvp\\pikucode-4e609bb8",
    );
    expect(result).toBe("pikucode-4e609bb8");
  });

  it("uses the final segment even when outside ~/.pikucode/worktrees", () => {
