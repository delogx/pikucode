...
Web uses the official `libghostty-vt` C ABI for parsing, terminal state, grapheme boundaries,
keyboard encoding, selection, and scrollback:

- Web loads a separately cached WebAssembly build and reads render state into a Canvas 2D surface.
- The artifact is built from the revision in
  `native/libghostty-vt/VERSION`.

The platform adapter deliberately owns only platform behavior. Web owns browser font shaping, the
hidden IME textarea, clipboard and DOM input, and its Canvas renderer. The web adapter also delegates application mouse encoding, word and line
...
`native/libghostty-vt/VERSION` is the single source of truth for the upstream pin (the upstream
`LICENSE` lives beside it). After bumping it, run:
...
artifacts, drift cannot hide, and there is no second pin to keep in sync. The same test enforces the artifact budget
