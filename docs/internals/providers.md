...
A provider is the agent runtime that does the actual work. Piku Code supports several, and the
orchestration layer does not know which one is behind a thread.

## Built-in drivers

[`builtInDrivers.ts`][drivers] exports `BUILT_IN_DRIVERS` with two entries:
...
`apps/server/src/provider/Layers/` (`CodexAdapter.ts` and `ClaudeAdapter.ts`) and conform to
