...
Piku Code uses [Vite+](https://viteplus.dev/guide/). Install the global `vp` command, install
...
- `vp run dev --browser`: Auto-opens a browser. Off by default. The dev runner writes
  `PIKU_NO_BROWSER` itself from this flag, so setting `PIKU_NO_BROWSER=0` in your environment has
...
- Pass dev-runner flags directly after the root task name, for example:
  `vp run dev --home-dir /tmp/piku-dev`

### Dev state directories

- Dev commands run from a linked **git worktree** default to that worktree's gitignored `.pikucode`, even
  when `PIKU_HOME` is set, storing state in `<worktree>/.pikucode/userdata`. Pass `--home-dir <path>` to
  choose another isolated directory explicitly. Submodules are not worktrees and keep the normal
  precedence.
- From the **main checkout**, dev commands implicitly use `~/.pikucode/dev`, keeping development state
  separate from `~/.pikucode/userdata`. An explicit `--home-dir <path>` stores state under
  `<path>/userdata`; the base directory remains available for caches, worktrees, and other shared
  data.

## Build, check, test

- `vp run build`: Fans out over `apps/*`, `packages/*`, `oxlint-plugin-pikucode`, and `scripts`.
...
- `node apps/server/scripts/piku-sqlite-state.ts <query|exec> --base-dir <path> ...`: Inspects or seeds
  an isolated Piku SQLite database; writes create a private backup first.

## Desktop artifacts

- `vp run dist:desktop:artifact --platform <mac|linux> --target <target> --arch <arch>`: Builds a desktop artifact for a specific platform/target/arch.
...
- Desktop production windows load the bundled UI from the `piku://app/` root URL (not a
  `127.0.0.1` document URL, and not an explicit `index.html` path).
- Desktop packaging includes `apps/server/dist` (the `piku` backend) and starts it on loopback with an
...
- Signed macOS builds also require `PIKU_APPLE_TEAM_ID` and
  `PIKU_MACOS_PROVISIONING_PROFILE`. The passkey RP domain is derived from
  `PIKU_CLERK_PUBLISHABLE_KEY` unless `PIKU_CLERK_PASSKEY_RP_DOMAINS` overrides it.
...
server, allowing the same bundle to work from localhost or any other hostname.
...
- Example: `PIKU_DEV_INSTANCE=branch-a vp run dev:desktop`

Offset resolution, in order:

1. `PIKU_PORT_OFFSET`, which must be a non-negative integer. Negative values are rejected.
2. `PIKU_DEV_INSTANCE`. An all-digit value is used directly as the offset; any other non-empty
