...
Piku Code has one server-side observability model:
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.pikucode/userdata/...` by default, or
`/custom/path/userdata/...` with `--home-dir /custom/path`), a linked worktree dev run uses
`<worktree>/.pikucode/userdata/logs/server.trace.ndjson`, and an implicit dev run outside a linked
worktree uses `~/.pikucode/dev/logs/server.trace.ndjson`.
...
```bash
export PIKU_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export PIKU_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export PIKU_OTLP_SERVICE_NAME=piku-local
```

Optional:

```bash
export PIKU_TRACE_MIN_LEVEL=Info
export PIKU_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `PIKU_OTLP_*`.

macOS app bundle example:

```bash
PIKU_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
PIKU_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
PIKU_OTLP_SERVICE_NAME=piku-desktop \
"/Applications/Piku Code.app/Contents/MacOS/Piku Code"
```
...
```bash
TRACE_FILE="${PIKU_HOME:-$HOME/.pikucode}/userdata/logs/server.trace.ndjson"
```

A dev server started from a linked worktree defaults to that worktree's local home:

```bash
TRACE_FILE="$WORKTREE/.pikucode/userdata/logs/server.trace.ndjson"
```

Only an implicit dev run outside a linked worktree uses the shared dev directory:

```bash
TRACE_FILE="$HOME/.pikucode/dev/logs/server.trace.ndjson"
```
...
- service name such as `piku-local`, `piku-dev`, or `piku-desktop`
...
- `piku_rpc_request_duration`
- `piku_orchestration_command_duration`
- `piku_orchestration_command_ack_duration`
- `piku_provider_turn_duration`
- `piku_git_command_duration`

Counters tell you volume and failure rate:

- `piku_rpc_requests_total`
- `piku_orchestration_commands_total`
- `piku_provider_turns_total`
- `piku_git_commands_total`
...
`piku_orchestration_command_ack_duration` measures:
...
1. Check `piku_orchestration_command_ack_duration` by `commandType`.
...
- `PIKU_OTLP_TRACES_URL` was not set
...
- `PIKU_TRACE_FILE`: override trace file path
- `PIKU_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `PIKU_TRACE_MAX_FILES`: rotated file count, default `10`
- `PIKU_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `PIKU_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `PIKU_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `PIKU_OTLP_TRACES_URL`: OTLP trace endpoint
- `PIKU_OTLP_METRICS_URL`: OTLP metric endpoint
- `PIKU_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `PIKU_OTLP_SERVICE_NAME`: service name, default `piku-server`
