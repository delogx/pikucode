...
Piku exposes V2 orchestration through its app-owned MCP endpoint. A provider
...
- create one or more ordinary top-level Piku threads;
...
These are Piku orchestration operations, not provider-native sub-agent APIs.
Delegated tasks always create a Piku child thread and run. The child receives
...
The provider-visible server key is `piku-code`. The endpoint registers both the
...
- the Piku environment;
- the parent Piku thread;
...
```text
-c mcp_servers.piku-code.url=http://127.0.0.1:<port>/mcp
-c mcp_servers.piku-code.bearer_token_env_var="PIKU_MCP_BEARER_TOKEN"
```

The provider-session token is placed in `PIKU_MCP_BEARER_TOKEN`. Both the
...
Creates a Piku-owned child thread and immediately dispatches the supplied task
prompt.
...
Creates between one and twenty ordinary top-level Piku threads:
