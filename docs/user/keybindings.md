...
The same configuration lives in `~/.pikucode/userdata/keybindings.json` on the machine running the
server, if you prefer editing it directly. Piku Code writes the built-in defaults into that file on
