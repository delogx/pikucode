...
This guide is for people who want to use more than one Claude setup in Piku Code. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Piku Code](./install.md).
...
In Piku Code Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Piku Code uses Claude Code's normal config directory.

When you set this field, Piku Code points Claude Code at that directory with the
...
`~/.claude_personal_home/.claude`, which is not where Piku Code looks.

Then add another Claude provider in Piku Code:
...
Piku Code only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Piku Code keeps separate config directories isolated
...
Add or edit a Claude provider in Piku Code Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Piku Code stores the value as a server secret and does not
...
Piku Code does not need a special Claude Code Router provider. Treat the router as a Claude
