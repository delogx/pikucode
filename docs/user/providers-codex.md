...
This guide is for people who want to use more than one Codex account in Piku Code. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Piku Code](./install.md).
...
- both accounts can see the same Piku/Codex sessions
...
In Piku Code Settings, name it something obvious:
...
In Piku Code Settings, add another Codex provider:
...
Piku Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Piku Code treats it
