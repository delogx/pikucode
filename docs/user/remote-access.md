...
Use this when you want to connect to a Piku Code server from another device such as a phone, tablet, or separate desktop app.
...
`piku pair` finds the running server (the shared `~/.pikucode` install, or the current worktree's dev server when run inside one), issues a one-time pairing token, and prints the pairing URL as a QR code you can scan from your phone.

If the server is only bound to loopback, the printed URL is not reachable from another device. Restart the server with a reachable `--host`, or connect the machine with Piku Connect (`npx piku connect`) so it is reachable through the managed tunnel.

Use `--ttl` to change the token lifetime and `--base-dir` to target a specific data directory.

If no server is running, `piku pair` says so and points you at `npx piku serve` or `npx piku connect`.

## Recommended Setup

Use a trusted private network that meshes your devices together, such as a VPN or overlay network.
...
- HTTPS/WSS-compatible endpoints work from `https://app.pikucode.dev`, but are not made the default
  automatically.
...
If the copied link points directly at `http://192.168.x.y:3773`, open it from a client that can reach that LAN address. If it points at `https://app.pikucode.dev/pair?...`, the hosted web app will save the environment and connect directly to the backend URL in the link.
...
Run the server with `piku serve`.

```bash
npx piku serve --host 0.0.0.0
```

`piku serve` starts the server without opening a browser and prints:
...
Use `piku serve --help` for the full flag reference. It supports the same general startup options as the normal server command, including an optional `cwd` argument.

For hosted web pairing, the backend has to be reachable over HTTPS. Use Piku Connect (`npx piku connect`)
or your own HTTPS endpoint in front of the server.
...
When the Piku Code web or desktop app and a remote server use different versions, a warning appears in
the conversation and in **Settings** → **Connections**. Follow the action shown there: Piku Code may
...
see [Keeping Piku Code in Sync](./updating.md).

On a Linux host, you can keep the server running after logout and manage it independently of the
connection method. See [Running Piku Code in the Background](./background-service.md).
...
1. `piku serve` issues a one-time owner pairing token.
...
The hosted web app at `https://app.pikucode.dev` can save a remote backend in browser local storage from a URL like:

```text
https://app.pikucode.dev/pair?host=https://backend.example.com:3773#token=PAIRCODE
```
...
Hosted pairing does not proxy traffic through Piku Code. The browser still connects directly to the backend URL in the pairing link.

## Managing Access Later

Use `piku auth` to manage access after the initial pairing flow.
...
Use `piku auth --help` and the nested subcommand help pages for the full reference.
...
- Use `piku auth` to revoke credentials or sessions you no longer trust.
