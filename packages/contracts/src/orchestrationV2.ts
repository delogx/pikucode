...
export const OrchestrationV2TurnItem = Schema.Union([
  ...
  Schema.Struct({
    ...OrchestrationV2TurnItemBaseFields,
    type: Schema.Literal("model_fallback"),
    // Provider-native model ids (e.g. `claude-fable-5`, `claude-opus-4-8`):
    // the model that refused and the model the provider re-ran the request on.
    fromModel: TrimmedNonEmptyString,
    toModel: TrimmedNonEmptyString,
    // Safety-classifier category from the refused response ("cyber", "bio",
    // …). Open string — providers ship new categories ahead of schema
    // updates. Null when the provider reported none.
