...
}).annotate({ description: "Links an authenticated cloud user to a Piku environment." });
...
>()("@piku/contracts/relay/RelayClientPrincipal") {}
...
>()("@piku/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in Piku Connect user.",
  ),
);
...
export const RelayPublicClientId = Schema.Literals(["piku-mobile", "piku-web"]);
export type RelayPublicClientId = typeof RelayPublicClientId.Type;
export const RelayMobileClientId = "piku-mobile" as const;
export const RelayWebClientId = "piku-web" as const;
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "Piku Code Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking Piku environments, connecting authorized clients, and publishing agent activity.",
  );
