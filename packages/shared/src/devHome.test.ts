...
import { resolveGitWorktreePath, resolveWorktreePikuHome } from "./devHome.ts";
...
const makeRepo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "piku-devhome-"));
      if (kind === "worktree") {
...
describe("resolveWorktreePikuHome", () => {
  it.effect("answers with .pikucode before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreePikuHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".pikucode"));
