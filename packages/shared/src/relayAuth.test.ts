...
describe("Clerk relay auth", () => {
  it("derives a custom Frontend API hostname from a Clerk publishable key", () => {
    expect(
      clerkFrontendApiHostnameFromPublishableKey(clerkPublishableKey("clerk.pikucode.dev")),
    ).toBe("clerk.pikucode.dev");
    expect(clerkFrontendApiUrlFromPublishableKey(clerkPublishableKey("clerk.pikucode.dev"))).toBe(
  ...
  it("reports semantic frontend API failures without inventing a cause", () => {
    ...
    const pathFrontendApi = "clerk.pikucode.dev/path";
  ...
  it("allows standard Clerk hosts and an exact configured custom hostname", () => {
    ...
    expect(isAllowedClerkFrontendApiHostname("clerk.pikucode.dev", "clerk.pikucode.dev")).toBe(
      true,
    );
    expect(isAllowedClerkFrontendApiHostname("attacker.example", "clerk.pikucode.dev")).toBe(false);
    expect(
      isAllowedClerkFrontendApiHostname("nested.clerk.pikucode.dev", "clerk.pikucode.dev"),
    ).toBe(false);
  });
});
