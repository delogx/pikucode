...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit PIKU_PORT_OFFSET when provided", () =>
      Effect.gen(function* () {
        const result = yield* resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "PIKU_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("returns structured context for a negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(error.configKey, "PIKU_PORT_OFFSET");
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("leaves the shared home implicit and disables browser auto-open", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.PIKU_HOME, undefined);
        assert.equal(env.PIKU_NO_BROWSER, "1");
      }),
    );
    ...
    it.effect("allows browser auto-open to be explicitly enabled", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.PIKU_NO_BROWSER, "0");
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.PIKU_HOME, path.resolve("/tmp/custom-piku"));
        assert.equal(env.PIKU_PORT, "4222");
        assert.equal(env.VITE_HTTP_URL, "http://localhost:4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.PIKU_NO_BROWSER, "1");
        assert.equal(env.PIKU_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.PIKU_LOG_WS_EVENTS, "1");
        assert.equal(env.PIKU_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            PIKU_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          pikuHome: undefined,
        ...
        assert.equal(env.PIKU_MODE, "web");
        assert.equal(env.PIKU_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.PIKU_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom pikuHome when provided", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const env = yield* createDevRunnerEnv({
          mode: "dev",
        ...
        assert.equal(env.PIKU_HOME, path.resolve("/tmp/my-piku"));
      }),
    );
    ...
    it.effect("pins desktop dev to a stable backend port and websocket url", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.PIKU_MODE, undefined);
        assert.equal(env.PIKU_NO_BROWSER, undefined);
        assert.equal(env.PIKU_HOST, undefined);
    ...
    it.effect("defaults dev server mode to the higher backend port range", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.PIKU_PORT, "13773");
        assert.equal(env.PORT, "5733");
      }),
    );
    ...
    for (const mode of ["dev", "dev:web"] as const) {
      it.effect(`leaves the client backend URLs unset in ${mode} mode`, () =>
        Effect.gen(function* () {
          ...
          assert.equal(env.PIKU_SINGLE_ORIGIN_DEV, "1");
        }),
      );
    }
    ...
    it.effect("clears the single-origin marker in dev:desktop mode", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev:desktop",
          baseEnv: { PIKU_SINGLE_ORIGIN_DEV: "1" },
          serverOffset: 0,
          webOffset: 0,
          pikuHome: undefined,
          browser: undefined,
        ...
        assert.equal(env.PIKU_SINGLE_ORIGIN_DEV, undefined);
    ...
    // --host configures the *backend* (PIKU_HOST). It must not become Vite's
  ...
  describe("devPortProbeHosts", () => {
    ...
    // actually bind, so --host/PIKU_HOST has to be probed as well.
    ...
    // Only the backend honours --host/PIKU_HOST. Vite reads HOST (set for
  ...
  describe("runDevRunnerWithInput", () => {
    it.effect("preserves invalid configuration as the exact cause", () =>
      Effect.gen(function* () {
          ...
          Effect.provide(
            Layer.merge(
              ...
              ConfigProvider.layer(
                ConfigProvider.fromEnv({ env: { PIKU_PORT_OFFSET: "not-an-integer" } }),
              ),
            ),
          ),
          Effect.flip,
        ...
        assert.deepStrictEqual(error.configKeys, ["PIKU_PORT_OFFSET", "PIKU_DEV_INSTANCE"]);
    ...
    // Base-dir precedence (--home-dir > worktree .pikucode > ambient PIKU_HOME)
    ...
    it.effect("ignores an ambient PIKU_HOME when no home is resolved", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: { PIKU_HOME: "/home/user/.pikucode" },
          serverOffset: 0,
          webOffset: 0,
          pikuHome: undefined,
          browser: undefined,
    ...
    describe("piku home precedence", () => {
      const makeWorktree = Effect.acquireRelease(
        Effect.sync(() => {
          const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "piku-devrunner-"));
      ...
      const spawnedHome = (input: {
        ...
        Effect.gen(function* () {
          ...
          yield* runDevRunnerWithInput({ ...devServerInput, pikuHome: input.pikuHome }).pipe(
            Effect.provide(Layer.mergeAll(emptyConfigLayer, netServiceLayer, spawnerLayer)),
            Effect.provideService(HostProcessPlatform, "linux"),
            Effect.provideService(HostProcessWorkingDirectory, input.cwd),
            Effect.provideService(
              HostProcessEnvironment,
              input.ambientHome === undefined ? {} : { PIKU_HOME: input.ambientHome },
            ),
          );

          return captured?.PIKU_HOME;
        });
      ...
      it.effect("prefers an explicit --home-dir over the worktree default", () =>
        Effect.gen(function* () {
          ...
          const home = yield* spawnedHome({
            pikuHome: "/tmp/explicit-home",
            cwd: root,
            ambientHome: "/home/user/.pikucode",
          });
      ...
      it.effect("treats a blank --home-dir as unset rather than as a selection", () =>
        Effect.gen(function* () {
          ...
          assert.equal(home, path.join(path.resolve(root), ".pikucode"));
        }).pipe(Effect.scoped),
      );

      it.effect("prefers the worktree .pikucode over an ambient PIKU_HOME", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const root = yield* makeWorktree;
          const home = yield* spawnedHome({
      ...
      it.effect("falls back to an ambient PIKU_HOME outside a worktree", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const home = yield* spawnedHome({
            pikuHome: undefined,
          ...
          assert.equal(home, path.resolve("/home/user/.pikucode"));
        }),
      );
